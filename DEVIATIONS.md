...
- Difference: Package is partially ported; exported surface currently includes core DOM/refs/viewport/scroll/focus helpers plus event/form/resize/load/drag/labeling/effect composables and all upstream test-file parity, but not the full upstream API set yet. Remaining unported upstream modules are `openLink.tsx`, `animation.ts`, and `useDeepMemo.ts`. Some composables are Vue lifecycle adaptations and do not yet mirror every React re-render timing edge case.
