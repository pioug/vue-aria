...
describe('TableNestedRows', () => {
  beforeEach(() => {
    tableViewPropsSpy.mockClear();
    treeGridPropsSpy.mockClear();
  });

  it('renders TreeGridTableView when nested rows are enabled and expandable rows are allowed', () => {
    ...
    expect(screen.getByTestId('tree-grid-table-view')).toBeTruthy();
    expect(screen.queryByTestId('table-view')).toBeNull();
    expect(treeGridPropsSpy).toHaveBeenCalledTimes(1);
  });
});
