...
export interface S2ToggleButtonGroupProps
...
function withGroupChildProps(children: VNode[], props: S2ToggleButtonGroupProps): VNode[] {
  ...
  return children.map((child) => {
    ...
    if (props.isEmphasized !== undefined && childProps.isEmphasized === undefined) {
      nextProps.isEmphasized = props.isEmphasized;
    }
...
export const ToggleButtonGroup = defineComponent({
  name: "S2ToggleButtonGroup",
  inheritAttrs: false,
  props: {
    size: {
  ...
  setup(props, { attrs, slots }) {
    const forwardedProps = computed(() => {
      ...
      return {
        ...merged,
        UNSAFE_className: clsx("s2-ToggleButtonGroup", merged.UNSAFE_className),
    ...
    return () =>
      h(SpectrumButtonGroup, forwardedProps.value as Record<string, unknown>, {
        default: () =>
          withGroupChildProps(
            slots.default?.() ?? [],
            props as S2ToggleButtonGroupProps
          ),
      });
  },
});
