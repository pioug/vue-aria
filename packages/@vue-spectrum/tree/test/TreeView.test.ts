...
async function pressEnter(target: { trigger: (event: string, options?: Record<string, unknown>) => Promise<unknown>, element: Element }) {
  ...
  await target.trigger("keyup", { key: "Enter" });
  await nextTick();
}
...
describe("TreeView", () => {
  ...
  it("does not expand rows from Enter when selection is enabled", async () => {
    const wrapper = renderTree({
      selectionMode: "single",
    });
  ...
  it('does not expand disabled rows from Enter when disabledBehavior is "all"', async () => {
    const onExpandedChange = vi.fn();
    const wrapper = renderTree({
      selectionMode: "none",
      disabledBehavior: "all",
  ...
  it('does not collapse disabled rows from Enter when disabledBehavior is "all"', async () => {
    const onExpandedChange = vi.fn();
    const wrapper = renderTree({
      selectionMode: "none",
      disabledBehavior: "all",
  ...
  it('does not toggle expansion from disabled row Enter when selection is enabled', async () => {
    const onExpandedChange = vi.fn();
    const onSelectionChange = vi.fn();
    const wrapper = renderTree({
      selectionMode: "multiple",
  ...
  it("does not expand rows from Enter when row actions are configured", async () => {
    const onAction = vi.fn();
    const wrapper = renderTree({
      selectionMode: "none",
      onAction,
  ...
  it("replaces single highlight selection across rows on pointer interactions", async () => {
    const onSelectionChange = vi.fn();
    const wrapper = renderTree({
      items: [
        { id: "alpha", name: "Alpha" },
  ...
  it("replaces highlight selection while moving focus with ArrowUp", async () => {
    const onSelectionChange = vi.fn();
    const wrapper = renderTree({
      items: [
        { id: "alpha", name: "Alpha" },
    ...
    await betaRow!.trigger("keydown", { key: "ArrowUp" });
    await nextTick();
    ...
    expect(betaRow!.attributes("aria-selected")).toBe("false");
    expect(alphaRow!.attributes("data-selected")).toBe("true");
    expect(betaRow!.attributes("data-selected")).toBeUndefined();
  });
  ...
  it("does not expand collapsed rows when activating row links with Enter", async () => {
    const onExpandedChange = vi.fn();
    const wrapper = mount(TreeView as any, {
      props: {
        "aria-label": "Collapsed keyboard linked tree",
        selectionMode: "none",
        onExpandedChange,
      },
      slots: {
