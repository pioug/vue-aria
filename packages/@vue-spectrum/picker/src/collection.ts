...
import type { PickerCollectionNode, SpectrumPickerNodeData } from "./types";
...
function createNode<T extends object>(
  ...
  childNodes: PickerCollectionNode<T>[] = []
): PickerCollectionNode<T> {
...
function normalizeDataNodes(
  ...
  parentKey: Key | null,
  keyPrefix: string
): PickerCollectionNode[] {
  const nodes: PickerCollectionNode[] = [];
  let index = 0;
...
function normalizeChildren(
  ...
  const createItemFromVNode = (vnode: VNode, fallbackKey: Key): PickerCollectionNode => {
  ...
  for (const vnode of children) {
    ...
    if (component === Section || component?.__spectrumPickerNodeType === "section") {
      const props = ((vnode.props ?? {}) as Record<string, unknown>) ?? {};
      const key = (vnode.key ?? props.key ?? props.id ?? fallbackKey) as Key;
    ...
    if (component === Item || component?.__spectrumPickerNodeType === "item") {
...
export function createPickerCollection(
  items: Iterable<SpectrumPickerNodeData> | undefined,
  slotChildren: VNode[]
): PickerCollectionNode[] {
...
export function getPickerDisabledKeys(nodes: Iterable<PickerCollectionNode>): Set<Key> {
  const disabledKeys = new Set<Key>();

  const visit = (node: PickerCollectionNode) => {
    if (node.type === "item") {
    ...
    for (const child of node.childNodes) {
      visit(child as PickerCollectionNode);
    }
  };
