...
import { dateFieldPropOptions } from "./types";
...
export const DateField = defineComponent({
  ...
  setup(props, { attrs, expose }) {
    ...
    const labelId = useId(undefined, "v-spectrum-datefield-label");
    const descriptionId = useId(undefined, "v-spectrum-datefield-description");
    const errorId = useId(undefined, "v-spectrum-datefield-error");
    ...
    const minValue = computed(() => formatDateValue(props.minValue));
    const maxValue = computed(() => formatDateValue(props.maxValue));
    const inputValue = computed(() => formatDateValue(currentValue.value));

    const setValue = (rawValue: string) => {
      const nextValue = parseDateValue(rawValue);
      if (!isControlled.value) {
          ...
          h("input", {
            ...
            "aria-invalid":
              Boolean(props.isInvalid) || props.validationState === "invalid"
                ? "true"
                : undefined,
            class: classNames("react-spectrum-DateField-input"),
            onInput: (event: Event) => {
