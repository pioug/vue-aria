...
import type { ListBoxCollectionNode, SpectrumListBoxNodeData } from "./types";
...
function createNode<T extends object>(
  ...
  childNodes: ListBoxCollectionNode<T>[] = []
): ListBoxCollectionNode<T> {
...
function normalizeDataNodes(
  items: Iterable<SpectrumListBoxNodeData>,
  level: number,
  parentKey: Key | null,
  keyPrefix: string
): ListBoxCollectionNode[] {
  const nodes: ListBoxCollectionNode[] = [];
  let index = 0;
...
function normalizeChildren(
  ...
  const createItemFromVNode = (vnode: VNode, fallbackKey: Key): ListBoxCollectionNode => {
  ...
  for (const vnode of children) {
    ...
    if (component === Section || component?.__spectrumListBoxNodeType === "section") {
      const props = ((vnode.props ?? {}) as Record<string, unknown>) ?? {};
      const key = (vnode.key ?? props.key ?? fallbackKey) as Key;
    ...
    if (component === Item || component?.__spectrumListBoxNodeType === "item") {
...
export function createListBoxCollection(
  items: Iterable<SpectrumListBoxNodeData> | undefined,
  slotChildren: VNode[]
): ListBoxCollectionNode[] {
