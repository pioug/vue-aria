...
vi.mock('@vue-aria/combobox', () => ({
  useComboBox: (props: Record<string, unknown>) => ({
    buttonProps: {
      'aria-label': 'Show suggestions'
    },
    inputProps: {
      role: 'combobox',
      'aria-label': String(props['aria-label'] ?? props.label ?? 'Combobox'),
      value: '',
      onChange: () => undefined
    },
    listBoxProps: {
...
vi.mock('@vue-spectrum/button', () => ({
  FieldButton: ({children, onPress, ...props}: {children?: React.ReactNode, onPress?: () => void} & Record<string, unknown>) => (
    React.createElement(
      'button',
      {
        ...props,
...
describe('ComboBox', () => {
  it('renders combobox input and trigger button', () => {
    render(
      React.createElement(
        ComboBox,
        {
    ...
    expect(screen.getByRole('button', {name: 'Show suggestions'})).toBeTruthy();
