...
describe('useProgressBar', function () {
  let renderProgressBarHook = (props: any) => {
  ...
  it('with custom text value', () => {
    let {progressBarProps} = renderProgressBarHook({value: 25, valueLabel: '¥25', 'aria-label': 'mandatory label'});
  ...
  it('with custom label', () => {
    let {progressBarProps, labelProps} = renderProgressBarHook({label: 'React test', value: 25});
