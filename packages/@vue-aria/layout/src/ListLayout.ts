...
import type {Collection, DropTarget, DropTargetDelegate, ItemDropTarget, Key, Node} from "./types";
import {getChildNodes} from "./collections";
import {InvalidationContext, Layout, LayoutInfo, Rect, Size} from "./virtualizer";
...
export class ListLayout<T, O extends ListLayoutOptions = ListLayoutOptions> extends Layout<Node<T>, O> implements DropTargetDelegate {
  ...
  protected buildLoader(node: Node<T>, x: number, y: number): LayoutNode {
    ...
    // room for the loader alongside a rendered emptyState
  ...
  protected buildSectionHeader(node: Node<T>, x: number, y: number): LayoutNode {
    ...
    return {
      layoutInfo: header,
      children: [],
      validRect: headerRect.intersection(this.requestedRect),
      node
    };
  }
  ...
  getDropTargetFromPoint(x: number, y: number, isValidDropTarget: (target: DropTarget) => boolean): DropTarget | null {
    ...
    if (!isValidDropTarget(target)) {
      if (y <= rect.y + rect.height / 2 && isValidDropTarget({...target, dropPosition: "before"} as ItemDropTarget)) {
        target.dropPosition = "before";
      } else if (isValidDropTarget({...target, dropPosition: "after"} as ItemDropTarget)) {
        target.dropPosition = "after";
      }
    } else if (y <= rect.y + 10 && isValidDropTarget({...target, dropPosition: "before"} as ItemDropTarget)) {
      target.dropPosition = "before";
    } else if (y >= rect.maxY - 10 && isValidDropTarget({...target, dropPosition: "after"} as ItemDropTarget)) {
      target.dropPosition = "after";
    }
  ...
  getDropTargetLayoutInfo(target: ItemDropTarget): LayoutInfo {
    ...
    if (target.dropPosition === "before") {
      ...
      if (targetNode) {
        ...
        while (currentKey != null) {
          ...
          layoutInfo.rect = this.layoutNodes.get(currentKey)?.layoutInfo.rect ?? layoutInfo.rect;
    ...
    return new LayoutInfo("dropIndicator", `${layoutInfo.key}:${target.dropPosition}`, rect);
  }
}
