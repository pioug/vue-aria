...
describe('Aria Table resizing', () => {
  it('returns expected slider input props for a resizer', () => {
    ...
    expect(result.current.inputProps.type).toBe('range');
    expect(result.current.inputProps.min).toBe(40);
    expect(result.current.inputProps.max).toBe(400);
    expect(result.current.inputProps.value).toBe(100);
    expect(result.current.inputProps['aria-label']).toBe('Resize Name');
    expect(result.current.inputProps['aria-labelledby']).toContain('table-id-name');
  });

  it('calls onResize when input value changes', () => {
    let state = createResizeState();
    let onResize = vi.fn();
    let inputRef = {current: null as HTMLInputElement | null};
    ...
    expect(sizes.get('name')).toBe(110);
  });
});
