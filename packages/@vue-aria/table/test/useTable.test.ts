...
describe('useTable', () => {
  it('sets the passed id', () => {
    let {getByTestId} = render(React.createElement(TestTable, {
      'aria-label': 'Table with id',
      id: 'table-id'
    }));
    expect(getByTestId('table').id).toBe('table-id');
  });

  it('returns grid semantics and aria label', () => {
    let {getByTestId} = render(React.createElement(TestTable, {
      'aria-label': 'Pokemon table'
    }));
    let table = getByTestId('table');
    expect(table.getAttribute('role')).toBe('grid');
    expect(table.getAttribute('aria-label')).toBe('Pokemon table');
  });
});
