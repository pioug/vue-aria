...
describe("useNumberField iPhone inputMode behavior", () => {
  it("uses text inputMode when negative values are possible", () => {
    ...
    expect(result.inputProps.inputMode).toBe("text");
    scope.stop();
  });

  it("uses decimal inputMode when negatives are disallowed and decimals are allowed", () => {
    ...
    expect(result.inputProps.inputMode).toBe("decimal");
    scope.stop();
  });

  it("uses numeric inputMode when negatives are disallowed and maxFractionDigits is 0", () => {
    const state = createState(0);
    const ref = { current: document.createElement("input") as HTMLInputElement | null };
    const scope = effectScope();
    const result = scope.run(() =>
