/*
 * Copyright 2020 Adobe. All rights reserved.
 * This file is licensed to you under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License. You may obtain a copy
 * of the License at http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS
 * OF ANY KIND, either express or implied. See the License for the specific language
 * governing permissions and limitations under the License.
 */

import React, {useRef} from 'react';
import {render} from '@testing-library/react';
import {useDialog} from '../src/useDialog';

vi.mock('@vue-aria/interactions', () => ({
  focusSafely: (el: HTMLElement | null) => el?.focus()
}));

vi.mock('@vue-aria/overlays', () => ({
  useOverlayFocusContain: () => {}
}));

function Example(props: any) {
  let ref = useRef<HTMLDivElement | null>(null);
  let {dialogProps} = useDialog(props, ref);
  return React.createElement('div', {ref, ...dialogProps, 'data-testid': 'test'}, props.children);
}

describe('useDialog', function () {
  it('should have role="dialog" by default', function () {
    let res = render(React.createElement(Example));
    let el = res.getByTestId('test');
    expect(el.getAttribute('role')).toBe('dialog');
  });

  it('should accept role="alertdialog"', function () {
    let res = render(React.createElement(Example, {role: 'alertdialog'}));
    let el = res.getByTestId('test');
    expect(el.getAttribute('role')).toBe('alertdialog');
  });

  it('should focus the overlay on mount', function () {
    let res = render(React.createElement(Example));
    let el = res.getByTestId('test');
    expect(el.getAttribute('tabindex')).toBe('-1');
    expect(document.activeElement).toBe(el);
  });

  it('should not focus the overlay if something inside is auto focused', function () {
    let res = render(
      React.createElement(
        Example,
        null,
        React.createElement('input', {'data-testid': 'input', autoFocus: true})
      )
    );
    let input = res.getByTestId('input');
    expect(document.activeElement).toBe(input);
  });
});
