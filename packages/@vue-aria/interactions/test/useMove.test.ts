...
import {installMouseEvent, installPointerEvent} from '@vue-aria/test-utils';
...
function Example(props: any) {
  ...
  return React.createElement('div', {tabIndex: -1, ...moveProps, 'data-testid': EXAMPLE_ELEMENT_TESTID}, props.children);
}
...
describe('useMove', function () {
  ...
  describe('user-select: none', function () {
  ...
  it('doesn\'t bubble to useMove on parent elements', function () {
    let eventsChild: any[] = [];
    let eventsParent: any[] = [];
    let addEventChild = e => eventsChild.push(e);
    let addEventParent = e => eventsParent.push(e);
    let tree = render(
      React.createElement(
        Example,
        {onMoveStart: addEventParent, onMove: addEventParent, onMoveEnd: addEventParent},
        React.createElement(Example, {onMoveStart: addEventChild, onMove: addEventChild, onMoveEnd: addEventChild})
      )
    );
  ...
  describe('keypresses', function () {
    let keycases: Array<{key: string, result: {deltaX: number, deltaY: number}}> = [
      {key: 'ArrowUp', result: {deltaX: 0, deltaY: -1}},
      {key: 'ArrowDown', result: {deltaX: 0, deltaY: 1}},
      {key: 'ArrowLeft', result: {deltaX: -1, deltaY: 0}},
      {key: 'ArrowRight', result: {deltaX: 1, deltaY: 0}}
    ];

    for (let {key, result} of keycases) {
      it(`responds to keypresses: ${key}`, function () {
        let events: any[] = [];
        let addEvent = e => events.push(e);
        let tree = render(
          React.createElement(Example, {
    ...
    it('allows handling other key events', function () {
      ...
      let tree = render(
        React.createElement(
          'div',
          {onKeyDown: e => addEvent({type: e.type, key: e.key})},
          React.createElement(Example, {
            onMoveStart: addEvent,
            onMove: addEvent,
            onMoveEnd: addEvent
