...
import type { AriaLabelingProps, DOMProps, ItemElement, ItemRenderer, Key, MultipleSelection, Orientation, StyleProps } from "@vue-types/shared";
...
export interface SpectrumActionGroupProps<T> extends AriaActionGroupProps<T>, StyleProps {
  ...
  isJustified?: boolean,
  /** Whether the ActionButtons should use the [quiet style](https://spectrum.adobe.com/page/action-button/#Quiet). */
  isQuiet?: boolean,
  /** The static color style to apply. Useful when the ActionGroup appears over a color background. */
  staticColor?: "white" | "black",
  /**
   * Defines when the ActionGroup when the buttons do not fit in the available space.
