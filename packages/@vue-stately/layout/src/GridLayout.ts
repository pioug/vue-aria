...
import type {DropTarget, DropTargetDelegate, ItemDropTarget, Key, Node} from "./types";
...
export class GridLayout<T, O extends GridLayoutOptions = GridLayoutOptions> extends Layout<Node<T>, O> implements DropTargetDelegate {
  ...
  protected get collection(): import("./types").Collection<Node<T>> {
  ...
  update(invalidationContext: InvalidationContext<O>): void {
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < numColumns; col++) {
        // Repeat skeleton cards until the end of the current row.
  ...
  getDropTargetFromPoint(x: number, y: number, isValidDropTarget: (target: DropTarget) => boolean): DropTarget {
    ...
    if (isValidDropTarget(target)) {
      ...
      if (pos < layoutInfoPos + 5 && isValidDropTarget({...target, dropPosition: "before"} as ItemDropTarget)) {
        target.dropPosition = "before";
      } else if (pos > layoutInfoPos + size - 5 && isValidDropTarget({...target, dropPosition: "after"} as ItemDropTarget)) {
        target.dropPosition = "after";
      }
    } else {
      // If dropping on the item isn't accepted, try the target before or after depending on the position.
      let mid = layoutInfoPos + size / 2;
      if (pos <= mid && isValidDropTarget({...target, dropPosition: "before"} as ItemDropTarget)) {
        target.dropPosition = "before";
