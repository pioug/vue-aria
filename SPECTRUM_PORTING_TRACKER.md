...
- Style readiness source of truth: `SPECTRUM_STYLE_TRACKER.md`
...
6. Ensure base component appearance is covered by shared Spectrum docs style baseline for usable parity and mark package `ready` in `SPECTRUM_STYLE_TRACKER.md`.
...
- In progress baseline: `@react-spectrum/slider` now has Vue `Slider`, `RangeSlider`, `SliderBase`, and `SliderThumb` primitives with baseline parity-style tests/docs and umbrella wiring (controlled/uncontrolled behavior, value output semantics, localized range-thumb labels, expanded controlled/uncontrolled clamp matrix coverage for min/max/step normalization, keyboard page/home/end coverage for single and range sliders including upstream-style default page-size snapping to step multiples across uneven ranges, upstream-style keyboard test helper parity via `test/utils`, disabled/tab-order focus behavior coverage for both single and range variants, plus-sign and custom `formatOptions` output coverage for single and range variants, track-click behavior including range nearest-thumb targeting and disabled suppression, enabled/disabled drag coverage for single and range thumbs, label-click focus behavior for range first-thumb targeting, and SSR coverage); advanced interaction and formatting parity remains.
...
- In progress baseline: `@react-spectrum/numberfield` now has Vue `NumberField` primitive with baseline parity-style tests/docs and umbrella wiring (numeric input semantics, min/max clamping, controlled/uncontrolled value handling, step-boundary snapping for stepped values across value/default normalization + blur commit + non-step increment recovery, blur-only commit when value changes plus decimal-step precision retention coverage, locale-aware parser behavior for currency/percent inputs with invalid partial-character rejection plus full-selection paste commit behavior and invalid non-empty blur reversion to the last valid formatted value, plus locale-specific grouped-number entry parity coverage for `es-ES`, composition-end invalid-input reversion parity, and expanded `beforeinput` affix-edit matrix parity (whole-symbol deletion allowed, partial-symbol edits prevented, numeric-region insertion/replacement allowed, history undo/redo allowed, and leading plus-sign deletion allowed) including broader delete/insert `inputType` coverage (`deleteContent`, `deleteByCut`, `deleteByDrag`, `deleteSoftLineBackward`, `deleteHardLineBackward`, `insertFromPaste`, `insertFromDrop`, `insertFromYank`, and `insertReplacementText`), stepper increment/decrement behavior including empty-field initial-step parity for unbounded/min-bounded/max-bounded flows plus pointer-type-aware stepper focus behavior (mouse -> input, non-mouse -> pressed control), typed out-of-range and min/max-step edge disable-state parity for steppers with clear-input re-enable coverage, upstream roledescription/label wiring parity (`aria-roledescription="Number field"`), long-press auto-repeat stepper behavior, upstream-style platform inputMode selection (including currency-without-decimals behavior), focused wheel-step behavior with zoom/horizontal guards, minus-sign empty-input blur handling without spurious `onChange` emission, custom increment/decrement aria-label support, hide-stepper support, hidden form-input support including hidden-input-only `name` wiring and null-reset value clearing parity, uncontrolled + controlled form-reset behavior (controlled reset restores the initial controlled value) plus controlled null-like reset-to-blank handling with matching hook-level reset regression coverage, `data-*` passthrough to the native input, component-ref DOM/focus helpers (`UNSAFE_getDOMNode`, `focus`), `validate(value)` support across ARIA/native modes, native required/invalid messaging lifecycle behavior (`checkValidity` display with valid-blur clearing and stepper-press validity commit clearing), function-style native `errorMessage` customizers, `Form.validationErrors` integration by field `name` including submit-driven native server-error flows with clear-on-input behavior, and SSR coverage); advanced locale nuances remain.
...
- In progress baseline: `@react-spectrum/table` now has Vue `TableView`, `Column`, `TableHeader`, `TableBody`, `Section`, `Row`, `Cell`, and `EditableCell` primitives with baseline static table semantics (`grid` + `rowgroup` + `row` + `columnheader` + `rowheader` + `gridcell`), expanded ARIA matrix parity (`aria-rowcount`, row/column index attributes, and unsorted-header `aria-sort` omission), keyboard row navigation, row-focus marshalling on non-focusable cell press, focused-child retention with left/right child-control cycling (including RTL ordering) and ArrowDown continuation after pressing interactive cell content, controlled/uncontrolled selection behavior, sort-state handling, render-phase static-slot parsing hardening (removing slot-invocation warning paths), and starter/SSR tests plus docs/umbrella wiring (including upstream-style `Table.test` / `Table.ssr.test` suite naming and shared `TableTests` fixture scaffolding parity); advanced upstream resizing/nested-row/virtualization and drag-and-drop parity remain.
...
- In progress baseline: `@react-spectrum/dropzone` now has Vue `DropZone` primitive with baseline drag/drop lifecycle wiring (`@vue-aria/dnd`), filled-state banner behavior with locale-aware default replace-message copy, and starter/SSR tests plus docs/umbrella wiring; advanced heading-context and full visual parity remain.
...
- [x] `@react-spectrum/illustratedmessage` -> `@vue-spectrum/illustratedmessage`
