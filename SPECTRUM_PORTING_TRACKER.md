...
- In progress baseline: `@react-spectrum/tag` now has Vue `TagGroup`, `Tag`, and compatibility `Item` primitives with baseline grid semantics (`grid`/`row`/`gridcell`), keyboard roving focus (LTR/RTL arrow behavior plus Home/End/PageUp/PageDown), tab entry/exit with remembered focused tag, disabled-key handling, removable-tag callbacks with focus handoff after removal (next tag or grid when empty), static slot composition support, baseline `maxRows` collapse/expand behavior (`Show all`/`Show less`), baseline action-area support (`actionLabel` + `onAction`), custom empty-state rendering support (`renderEmptyState`), baseline field semantics (`label`/`description`/`errorMessage`) including `Form.validationErrors` integration by field `name`, and baseline tag item passthrough support for data attributes + link-style tags (`href`) + static class overrides (`UNSAFE_className`) with starter test/docs/umbrella wiring; advanced visual/theming parity remains.
