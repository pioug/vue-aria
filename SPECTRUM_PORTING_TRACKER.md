...
- In progress baseline: `@react-spectrum/card` now has Vue `Card`, `CardView`, and layout class primitives with baseline tests/docs plus keyboard navigation (`Arrow*`, `Home`/`End`), selection state handling (`selectedKeys`/`defaultSelectedKeys`/`disabledKeys`/`onSelectionChange`), single/multiple selection coverage, checkbox interaction behavior, `selectionMode="none"` behavior checks, and falsy-id coverage; advanced layout and interaction parity is still pending.
