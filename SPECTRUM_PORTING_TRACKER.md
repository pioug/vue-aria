...
- In progress baseline: `@react-spectrum/toast` now has Vue `ToastContainer` and `ToastQueue` primitives with baseline global queue helpers, timeout and action/close behavior, programmatic close callbacks returned from queue APIs, single-active-container rendering, focus-restore coverage when dismissing toasts back to previously focused elements, and starter/SSR tests plus docs/umbrella wiring; advanced icon/transition/focus-management parity remains.
