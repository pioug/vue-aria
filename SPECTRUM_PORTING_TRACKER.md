...
- In progress baseline: `@react-spectrum/tag` now has Vue `TagGroup`, `Tag`, and compatibility `Item` primitives with baseline grid semantics (`grid`/`row`/`gridcell`), keyboard roving focus (LTR/RTL arrow behavior plus Home/End/PageUp/PageDown), disabled-key handling, removable-tag callbacks with focus handoff after removal (next tag or grid when empty), static slot composition support, baseline `maxRows` collapse/expand behavior (`Show all`/`Show less`), baseline action-area support (`actionLabel` + `onAction`), custom empty-state rendering support (`renderEmptyState`), and baseline tag item passthrough support for data attributes + link-style tags (`href`) with starter test/docs/umbrella wiring; field-validation integration and advanced visual/theming parity remain.
