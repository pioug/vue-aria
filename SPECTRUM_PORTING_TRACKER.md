...
- [ ] `Menu` — `4 / 5` exported; missing: `SubmenuTrigger`
...
- In progress baseline: `@react-spectrum/menu` now has Vue `Menu`, `MenuSection`, `MenuItem`, `MenuTrigger`, and `ActionMenu` primitives with baseline trigger/menu interactions (open/close, keyboard navigation, click/keyboard item activation, controlled/default open state, selection-state handling for `none`/`single`/`multiple`, and section grouping support) with starter test/docs/umbrella wiring; advanced submenu/contextual-help and full overlay positioning parity remain.
