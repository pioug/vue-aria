...
- In progress baseline: `@react-spectrum/steplist` now has Vue `StepList` and `StepListItem` primitives with baseline progression and selection-state behavior (completed-step gating, controlled/uncontrolled selected step handling, disabled/read-only handling, static slot composition support, and SSR/docs/umbrella wiring); advanced localized state-message parity remains.
