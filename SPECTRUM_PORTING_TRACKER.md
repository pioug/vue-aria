...
- In progress baseline: `@react-spectrum/tooltip` now has Vue `Tooltip` and `TooltipTrigger` primitives with baseline focus/hover trigger behavior (including default hover-delay parity (1500ms), default hover close-delay parity (500ms), configurable hover timing via `delay`/`closeDelay`, focus-overrides-delay coverage, focus-only trigger mode (`trigger="focus"`), single-visible-tooltip coordination across multiple triggers, and warmup/cooldown reopen parity where recently shown delayed tooltips reopen immediately within cooldown), Escape/press/scroll close handling (including global keyboard-dismiss from outside trigger focus, scroll-dismiss parity, combined focus+hover close edge cases, and `shouldCloseOnPress=false` coverage for pointer + keyboard paths), controlled/uncontrolled visibility flows (`isOpen` / `defaultOpen`), disabled-trigger behavior (`isDisabled`), trigger-anchored placement positioning support, and starter/SSR tests plus docs/umbrella wiring; advanced icon/transition and full overlay parity remains.
