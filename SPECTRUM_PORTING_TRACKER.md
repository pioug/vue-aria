...
- In progress baseline: `@react-spectrum/textfield` now has Vue `TextField` and `TextArea` primitives with label/help/error semantics, controlled/uncontrolled value handling, placeholder deprecation warning parity, validation behavior coverage (`aria` + `native`), textarea auto-resize parity based on content `scrollHeight`, and SSR/docs/umbrella wiring; pending parity includes advanced icon/loading behavior and additional interaction edge cases.
...
- In progress baseline: `@react-spectrum/tooltip` now has Vue `Tooltip` and `TooltipTrigger` primitives with baseline focus/hover trigger behavior, Escape/press close handling, trigger-anchored placement positioning support, and starter/SSR tests plus docs/umbrella wiring; advanced icon/transition and full overlay parity remains.
