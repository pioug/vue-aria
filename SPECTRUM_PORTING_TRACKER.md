...
- In progress baseline: `@react-spectrum/actionbar` now has Vue `ActionBar`, `ActionBarItem`, and `ActionBarContainer` primitives with baseline selection-count open/close behavior, clear-selection handling (`clear` action button + Escape key), `ActionGroup` action wiring, live selection announcements via `role="status"`, and static slot composition support with starter test/docs/umbrella coverage; transition/focus-restore parity remains.
