...
- In progress baseline: `@react-spectrum/dialog` now has Vue `Dialog`, `AlertDialog`, `DialogTrigger`, and `DialogContainer` primitives with baseline trigger/container wiring, dismiss/escape behavior, alert-dialog action flows, mobile popover fallback parity in `DialogTrigger` (`popover` -> `modal` by default on mobile with `mobileType` override) with forced modal dismissability matching upstream fallback behavior, expanded `DialogTrigger` dismissal parity (outside-click gating across overlay modes plus hidden dismiss-button close with focus restoration), expanded `DialogContainer` dismissal semantics (Escape close, keyboard-dismiss-disabled behavior, and outside-click dismissal gating via `isDismissable`), custom portal-container targeting support across trigger/container overlays (`container`), and `Dialog` label hardening (`aria-labelledby` / `aria-label` explicit-prop precedence plus automatic first-heading linkage when no explicit label props are provided) plus starter/SSR tests and docs/umbrella wiring; advanced overlay/context parity remains.
