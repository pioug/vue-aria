...
- Active package lock (single-WIP rule): `@react-spectrum/contextualhelp` in `SPECTRUM_WIP.md`.
...
- Parity-complete baseline: `@react-spectrum/inlinealert` now has Vue `InlineAlert` primitive with upstream-equivalent alert semantics, variant classes, localized variant icon labels, autoFocus behavior, and parity docs/tests plus umbrella wiring.
...
- [x] `@react-spectrum/inlinealert` -> `@vue-spectrum/inlinealert`
