...
- In progress baseline: `@react-spectrum/actionbar` now has Vue `ActionBar`, `ActionBarItem`, and `ActionBarContainer` primitives with baseline selection-count open/close behavior, clear-selection handling (`clear` action button + Escape key), `ActionGroup` action wiring, live selection announcements via `role="status"`, focus restore to the pre-toolbar element on close, close-transition lifecycle wiring, and static slot composition support with starter test/docs/umbrella coverage; richer animation/collapse parity remains.
- In progress baseline: `@react-spectrum/actiongroup` now has Vue `ActionGroup` and `ActionGroupItem` primitives with baseline item rendering and selection semantics (single/multiple state handling, disabled-key behavior, orientation classes, roving keyboard focus including RTL handling, and static slot composition support), plus baseline `overflowMode="collapse"` behavior (overflow items moved into a menu trigger, and full collapse when selection mode is enabled and not all items fit) with starter test/docs/umbrella wiring; icon-label collapse behavior remains.
