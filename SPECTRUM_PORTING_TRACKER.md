...
- In progress baseline: `@react-spectrum/inlinealert` now has Vue `InlineAlert` primitive with baseline alert semantics, variant class behavior, variant-specific icon semantics (`info`/`positive`/`notice`/`negative` with accessible labels), autofocus handling, and starter/SSR tests plus docs/umbrella wiring; advanced icon/theming parity remains.
- In progress baseline: `@react-spectrum/tooltip` now has Vue `Tooltip` and `TooltipTrigger` primitives with baseline focus/hover trigger behavior (including default hover-delay parity (1500ms), default hover close-delay parity (500ms), configurable hover timing via `delay`/`closeDelay`, focus-overrides-delay coverage, focus-only trigger mode (`trigger="focus"`), single-visible-tooltip coordination across multiple triggers, and warmup/cooldown parity including immediate reopen within cooldown and no warmup carryover when a trigger is left before delayed open completes), Escape/press/scroll close handling (including global keyboard-dismiss from outside trigger focus, scroll-dismiss parity, combined focus+hover close edge cases, and `shouldCloseOnPress=false` coverage for pointer + keyboard paths), controlled/uncontrolled visibility flows (`isOpen` / `defaultOpen`, including concurrent controlled-open behavior), trigger `aria-describedby` semantics while open, tooltip ref parity via `UNSAFE_getDOMNode`, disabled-trigger behavior (`isDisabled`), portal-container parity through `UNSAFE_PortalProvider` (including nested null override reset semantics), trigger-anchored placement positioning support (including logical `start`/`end` direction mapping), and starter/SSR tests plus docs/umbrella wiring; advanced icon/transition and full overlay parity remains.
- In progress baseline: `@react-spectrum/contextualhelp` now has Vue `ContextualHelp` primitive with baseline quiet-trigger popover behavior, default/custom ARIA label handling, placement support, trigger-ref parity (`UNSAFE_getDOMNode`/`focus()` to the action button), and portal-container routing parity via `UNSAFE_PortalProvider` (including nested null override reset behavior), plus starter/SSR tests and docs/umbrella wiring; advanced icon/animation parity remains.
- In progress baseline: `@react-spectrum/toast` now has Vue `ToastContainer` and `ToastQueue` primitives with baseline global queue helpers, timeout and action/close behavior (including hover/focus timer pause-resume), programmatic close callbacks returned from queue APIs, single-active-container rendering (including rerender stability across container topology changes), focus-restore coverage when dismissing toasts back to previously focused elements, multi-toast focus-handoff coverage on dismissal, `F6` keyboard focus transfer into the toast region (including focused-region `focus-ring` class behavior), custom toast-event interception coverage including detail payload parity, portal-container routing parity via `UNSAFE_PortalProvider` (including nested null override reset behavior), and starter/SSR tests plus docs/umbrella wiring; advanced icon/transition parity remains.
