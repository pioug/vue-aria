...
- `@vue-spectrum/dialog` baseline is now in progress with initial `Dialog`, `AlertDialog`, `DialogTrigger`, and `DialogContainer` ports, including baseline trigger/container overlay wiring, dismiss/escape handling, alert-dialog action behavior, mobile popover fallback parity in `DialogTrigger` (`popover` -> `modal` by default on mobile with `mobileType` override), expanded `DialogTrigger` dismissal parity (outside-click gating across overlay modes plus hidden dismiss-button close with focus restoration), and expanded `DialogContainer` dismissal semantics (Escape close, keyboard-dismiss-disabled behavior, outside-click dismissal gating via `isDismissable`) with starter/SSR tests plus docs/umbrella wiring; advanced context/overlay integration parity remains.
