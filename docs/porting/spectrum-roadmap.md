...
- `@vue-spectrum/actionbar` baseline is now in progress with initial `ActionBar`, `ActionBarItem`, and `ActionBarContainer` ports, including selection-count-driven open/close behavior, clear-selection handling (button + Escape), `ActionGroup` action wiring, live selection announcements, and static slot composition support with docs and umbrella wiring; transition/focus-restore parity remains.
