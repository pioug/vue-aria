...
- `@vue-spectrum/s2` baseline is now in progress with initial infrastructure utility ports (`pressScale`, `isDocsEnv`) and first component primitives (`Provider`, `ActionMenu`, `Button`, `LinkButton`, `ActionButton`, `ActionButtonGroup`, `ButtonGroup`, `ComboBox`, `ColorField`, `CloseButton`, `DateField`, `NumberField`, `SearchField`, `SelectBox`, `SelectBoxGroup`, `Slider`, `Switch`, `TextField`, `ToggleButton`, `ToggleButtonGroup`) plus starter tests/docs and umbrella wiring; full S2 component export parity remains.
