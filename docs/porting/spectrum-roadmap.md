...
- `@vue-spectrum/steplist` baseline is now in progress with initial `StepList` and `StepListItem` ports, including baseline progression and selection-state behavior (completed-step gating, controlled/uncontrolled selected step handling, disabled/read-only behavior, and static slot composition support) with docs and umbrella wiring; advanced localized state-message parity remains.
