...
- `@vue-spectrum/button` baseline is now in progress with initial `Button`, `ActionButton`, `ClearButton`, `LogicButton`, and `ToggleButton` ports plus starter parity tests (press handling, disabled behavior, focus behavior, anchor rendering, and toggle selection state); pending parity includes upstream pending-state behavior, icon/slot behavior nuances, and deeper interaction edge cases.
