...
- `@vue-spectrum/overlays` baseline is now in progress with `Overlay`, `Popover`, `Modal`, and `OpenTransition` ports, including portal mounting behavior, transition lifecycle callback wiring, Popover positioning/dismiss semantics (`Escape` close, modal underlay + non-modal mode, arrow visibility toggles), and Modal overlay semantics (`Escape` close, optional outside-dismiss, underlay, and modal scroll-lock/focus-containment behavior) with starter/SSR tests plus docs and umbrella wiring; `Tray` parity remains.
