...
- `@vue-spectrum/s2` baseline is now in progress with initial infrastructure utility ports (`pressScale`, `isDocsEnv`) and first component primitives (`Provider`, `ActionMenu`, `Button`, `LinkButton`, `ActionButton`, `ActionButtonGroup`, `ButtonGroup`, `ComboBox`, `ColorField`, `CloseButton`, `DateField`, `NumberField`, `SearchField`, `SelectBox`, `SelectBoxGroup`, `Slider`, `Switch`, `TextField`, `ToggleButton`, `ToggleButtonGroup`) plus starter tests/docs and umbrella wiring; recent parity hardening adds keyboard navigation coverage for `SelectBoxGroup` (Arrow/Home/End focus flow, keyboard selection, and disabled-option skipping), while full S2 component export parity remains.
