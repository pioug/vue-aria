...
- `@vue-spectrum/card` baseline is now in progress with initial `Card`, `CardView`, and layout class ports plus parity-aligned starter tests (including keyboard-nav, `Home`/`End`, single/multiple selection handling, checkbox interaction, `selectionMode="none"` behavior, and falsy-id coverage); advanced interaction/layout parity remains.
