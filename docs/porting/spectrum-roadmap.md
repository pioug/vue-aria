...
- `@vue-spectrum/s2` baseline is now in progress with initial infrastructure utility ports (`pressScale`, `isDocsEnv`) plus starter tests/docs and umbrella wiring; full S2 component export parity remains.
...
2. Expand the `s2` package from infrastructure baseline into component-surface parity, starting with shared provider/context and high-usage control primitives.
...
4. Continue `@vue-spectrum/s2` by porting the first component primitives (`Provider`, `Button`) next.
