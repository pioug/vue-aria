...
- `@vue-spectrum/menu` baseline is now in progress with initial `Menu`, `MenuSection`, `MenuItem`, `MenuTrigger`, and `ActionMenu` ports, including trigger/menu open-close behavior, keyboard navigation (`Arrow*`, `Home`, `End`, `Enter`, `Space`, `Escape`), single/multiple selection-state handling, and section grouping support with docs and umbrella wiring; advanced submenus/contextual-help and full popover positioning parity remain.
