...
- `@vue-spectrum/menu` baseline is now in progress with initial `Menu`, `MenuSection`, `MenuItem`, `SubmenuTrigger`, `MenuTrigger`, and `ActionMenu` ports, including trigger/menu open-close behavior, keyboard navigation (`Arrow*`, `Home`, `End`, `Enter`, `Space`, `Escape`), single/multiple selection-state handling, section grouping support, and submenu trigger behavior with docs and umbrella wiring; advanced contextual-help integration and full popover positioning parity remain.
