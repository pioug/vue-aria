...
- `@vue-spectrum/picker` baseline is now in progress with initial `Picker`, `PickerItem`, and `PickerSection` ports, including trigger/listbox open-close behavior, trigger-anchored popover placement support, keyboard navigation (`Arrow*`, `Home`, `End`, `Enter`, `Space`, `Escape`), controlled/uncontrolled selection behavior, and static slot composition support with docs and umbrella wiring; advanced tray/mobile behavior, async loading, and full form/validation parity remain.
