...
- `@vue-spectrum/s2` baseline is now in progress with initial infrastructure utility ports (`pressScale`, `isDocsEnv`) and first component primitives (`Provider`, `ActionMenu`, `Button`, `LinkButton`, `ActionButton`, `ActionButtonGroup`, `ButtonGroup`, `ColorField`, `CloseButton`, `SearchField`, `SelectBox`, `SelectBoxGroup`, `ToggleButton`, `ToggleButtonGroup`) plus starter tests/docs and umbrella wiring; full S2 component export parity remains.
...
4. Continue `@vue-spectrum/s2` by porting the next component primitives (`DateField`, `NumberField`) and deepening grouped-control behavior parity next.
