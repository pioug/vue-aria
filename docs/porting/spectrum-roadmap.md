...
- `@vue-spectrum/overlays` baseline is now in progress with `Overlay`, `Popover`, and `OpenTransition` ports, including portal mounting behavior, transition lifecycle callback wiring, and Popover positioning/dismiss semantics (`Escape` close, modal underlay + non-modal mode, arrow visibility toggles) with starter/SSR tests plus docs and umbrella wiring; `Modal`/`Tray` parity remains.
