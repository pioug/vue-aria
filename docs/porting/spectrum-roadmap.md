...
- `@vue-spectrum/steplist` baseline is now in progress with initial `StepList` and `StepListItem` ports, including baseline progression and selection-state behavior (completed-step gating, controlled/uncontrolled selected step handling, disabled/read-only behavior, static slot composition support, and localized step-state messages) with docs and umbrella wiring; advanced icon visual parity remains.
...
- `@vue-spectrum/actionbar` baseline is now in progress with initial `ActionBar`, `ActionBarItem`, and `ActionBarContainer` ports, including selection-count-driven open/close behavior, clear-selection handling (button + Escape), `ActionGroup` action wiring, and static slot composition support with docs and umbrella wiring; transition/announcement parity remains.
...
- `@vue-spectrum/picker` baseline is now in progress with initial `Picker`, `PickerItem`, and `PickerSection` ports, including trigger/listbox open-close behavior, keyboard navigation (`Arrow*`, `Home`, `End`, `Enter`, `Space`, `Escape`), controlled/uncontrolled selection behavior, and static slot composition support with docs and umbrella wiring; advanced popover/tray, async loading, and full form/validation parity remain.
- `@vue-spectrum/menu` baseline is now in progress with initial `Menu`, `MenuSection`, `MenuItem`, `SubmenuTrigger`, `MenuTrigger`, and `ActionMenu` ports, including trigger/menu open-close behavior, keyboard navigation (`Arrow*`, `Home`, `End`, `Enter`, `Space`, `Escape`), single/multiple selection-state handling, section grouping support, submenu trigger behavior, and ActionMenu trigger-label/aria/autofocus parity coverage with docs and umbrella wiring; advanced contextual-help integration and full popover positioning parity remain.
