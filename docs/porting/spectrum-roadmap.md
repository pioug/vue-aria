...
- `@vue-spectrum/dialog` baseline is now in progress with initial `Dialog`, `AlertDialog`, `DialogTrigger`, and `DialogContainer` ports, including baseline trigger/container overlay wiring, dismiss/escape handling, alert-dialog action behavior, and mobile popover fallback parity in `DialogTrigger` (`popover` -> `modal` by default on mobile with `mobileType` override) with starter/SSR tests plus docs/umbrella wiring; advanced context/overlay integration parity remains.
