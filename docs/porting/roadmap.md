...
- Drag/drop clipboard + keyboard target baseline: `useClipboard`, `writeToDataTransfer`, `readFromDataTransfer`, `DragTypes`, `navigate`, `ListDropTargetDelegate`
...
- Drag/drop clipboard + keyboard target baselines in place (`@vue-aria/dnd` `useClipboard`, `navigate`)
