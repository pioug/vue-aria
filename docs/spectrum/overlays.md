...
- Baseline now includes `Popover` with `@vue-aria/overlays` positioning/dismiss wiring (`Escape` close, modal-underlay press close behavior, close-on-blur focus-within behavior, optional non-modal mode, hidden dismiss buttons, and arrow visibility toggle).
