...
- Baseline includes combobox/input/listbox ARIA wiring, type-to-filter behavior, button-trigger opening, selection commit behavior, placeholder deprecation warning parity, native form wiring (`name`/`form`) plus `formValue="key"` hidden-input submission behavior, and static slot composition support via `ComboBoxItem` and `ComboBoxSection`.
