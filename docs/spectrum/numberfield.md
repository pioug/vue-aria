...
- Baseline now includes `validate(value)` support across ARIA/native validation modes, native required/invalid messaging lifecycle behavior (`checkValidity` display with valid-blur clearing and stepper-press validity commit clearing), function-style native `errorMessage` customizers, and `Form.validationErrors` integration by field `name` (including submit-driven native server error behavior) with clear-on-input behavior.
