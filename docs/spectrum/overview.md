...
- `@vue-spectrum/picker` (in progress): picker primitives (`Picker`, `PickerItem`, `PickerSection`) with baseline trigger/listbox interaction, trigger-anchored placement support, keyboard navigation, controlled/uncontrolled selection behavior, async loading (`isLoading`/`onLoadMore`), and static slot composition support
