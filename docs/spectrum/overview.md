...
- `@vue-spectrum/picker` (in progress): picker primitives (`Picker`, `PickerItem`, `PickerSection`) with baseline trigger/listbox interaction, trigger-anchored placement support, mobile tray rendering support, keyboard navigation, controlled/uncontrolled selection behavior, async loading (`isLoading`/`onLoadMore`), basic form submission/reset wiring (`name`/`form`), and static slot composition support
