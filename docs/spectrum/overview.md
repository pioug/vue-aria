...
- Priority mode: React Spectrum v1 package parity first; S2 remains a secondary track until v1 package and quality-gate parity is complete.
...
- `@vue-spectrum/s2` (in progress, secondary track): early S2 baseline with shared utility exports (`pressScale`, `isDocsEnv`) plus first component primitives (`Provider`, `Accordion`, `Disclosure`, `DisclosurePanel`, `DisclosureTitle`, `ActionBar`, `ActionMenu`, `AlertDialog`, `Breadcrumbs`, `Breadcrumb`, `BreadcrumbItem`, `Button`, `LinkButton`, `ActionButton`, `ActionButtonGroup`, `Avatar`, `Badge`, `ButtonGroup`, `ComboBox`, `ColorArea`, `ColorField`, `ColorSlider`, `ColorSwatch`, `ColorSwatchPicker`, `ColorWheel`, `Heading`, `Header`, `Content`, `Text`, `Keyboard`, `Footer`, `Checkbox`, `CheckboxGroup`, `CloseButton`, `ContextualHelp`, `DateField`, `DatePicker`, `DateRangePicker`, `TimeField`, `Dialog`, `DialogTrigger`, `DialogContainer`, `useDialogContainer`, `Divider`, `DropZone`, `Form`, `Image`, `IllustratedMessage`, `InlineAlert`, `Link`, `Meter`, `Menu`, `MenuItem`, `MenuTrigger`, `NumberField`, `ProgressBar`, `ProgressCircle`, `SearchField`, `TableView`, `TableHeader`, `TableBody`, `Column`, `Row`, `Cell`, `Radio`, `RadioGroup`, `SelectBox`, `SelectBoxGroup`, `Slider`, `RangeSlider`, `StatusLight`, `Switch`, `TagGroup`, `Tag`, `TextField`, `TextArea`, `ToggleButton`, `ToggleButtonGroup`, `Tooltip`, `TooltipTrigger`, `Well`); remaining S2 parity is deferred until v1 package parity is complete.
- `@vue-spectrum/icon`: icon wrapper component primitives (`Icon`, `UIIcon`, `Illustration`) with slot-prop override parity for `icon` and `illustration` slots
- `@vue-spectrum/utils`: shared utility baseline with class-name helpers, slot/context utilities, and style/DOM/media helpers
...
- `@vue-spectrum/actionbar` (in progress): action-bar primitives (`ActionBar`, `ActionBarItem`, `ActionBarContainer`) with baseline selected-count rendering, clear-selection/action wiring, live selection announcements, focus-restore on close, close-transition lifecycle wiring, and static slot composition support
...
- `@vue-spectrum/tag` (in progress): tag primitives (`TagGroup`, `Tag`) with baseline grid semantics, roving focus, removable-tag behavior, and static slot composition support
- `@vue-spectrum/picker` (in progress): picker primitives (`Picker`, `PickerItem`, `PickerSection`) with baseline trigger/listbox interaction, trigger-anchored placement support, mobile tray rendering support, keyboard navigation, controlled/uncontrolled selection behavior, async loading (`isLoading`/`onLoadMore`), basic form submission/reset wiring (`name`/`form`), basic validation semantics (`isRequired`/`isInvalid`/`validationState`), and static slot composition support
- `@vue-spectrum/menu` (in progress): menu primitives (`Menu`, `MenuSection`, `MenuItem`, `SubmenuTrigger`, `MenuTrigger`, `ActionMenu`) with baseline trigger/menu interaction, trigger-anchored placement support, section-grouping support, submenu trigger behavior, ActionMenu aria/autofocus coverage, and selection behavior
...
- `@vue-spectrum/contextualhelp` (in progress): contextual-help primitive (`ContextualHelp`) with baseline trigger/popover rendering, default/custom label behavior, and placement support
...
- `@vue-spectrum/combobox` (in progress): combobox primitives (`ComboBox`, `ComboBoxItem`, `ComboBoxSection`) with baseline input/listbox ARIA wiring, filter-on-type behavior, controlled/uncontrolled selection/input/open state handling, and static slot composition support
- `@vue-spectrum/autocomplete` (in progress): search autocomplete primitives (`SearchAutocomplete`, `SearchAutocompleteItem`, `SearchAutocompleteSection`) with baseline combobox/search/listbox ARIA wiring, type-to-filter behavior, clear-button semantics, submit callbacks, loading/load-more handling, and static slot composition support
