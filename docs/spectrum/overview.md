...
- `@vue-spectrum/numberfield` (in progress): numeric input primitive (`NumberField`) with controlled/uncontrolled value support, clamped min/max behavior, upstream-style platform `inputMode` behavior, focused wheel stepping guards, and baseline stepper interactions
