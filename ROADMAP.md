...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` docs parity update:
  - added invalid-state and custom `errorMessage` examples for both `DatePicker` and `DateRangePicker`.
    - `docs/packages/spectrum-datepicker.md`
- Additional `@vue-spectrum/datepicker` parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added integration coverage for `pageBehavior` passthrough in multi-month calendar/range-calendar overlay navigation.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 30 tests).
- Additional `@vue-spectrum/datepicker` docs parity update:
...
- Additional `@vue-spectrum/datepicker` parity update:
  - added integration coverage for `minValue` / `maxValue` boundary passthrough in both datepicker overlay calendar variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 43 tests).
- Additional `@vue-spectrum/datepicker` parity update:
  - added `allowsNonContiguousRanges` passthrough coverage in `DateRangePicker`, including default contiguous clamping and explicit non-contiguous enablement paths.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 45 tests).
- Additional `@vue-spectrum/datepicker` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added controlled-open interaction coverage ensuring click-driven `onOpenChange` requests emit while overlays remain closed until controlled `isOpen` prop updates.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 51 tests).
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added form-serialization regression coverage to ensure hidden `name` / `startName` / `endName` inputs update after interactive date and range selection.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 57 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added placeholder-state coverage for custom placeholder text/class rendering and post-selection clearing in `DatePicker` and `DateRangePicker`.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 59 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - expanded SSR coverage for `description` rendering in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.ssr.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added provider override-precedence coverage confirming explicit local props supersede provider defaults for disabled and validation-state behavior.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - extended focus-transition regression coverage to assert `onFocus` and `onBlur` callback forwarding in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 79 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added keyboard callback regression coverage for `onKeyUp` forwarding and open-state suppression in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - expanded SSR coverage for callback-based `errorMessage` rendering in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.ssr.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 93 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added controlled keyboard-open request coverage ensuring `Alt+ArrowDown` emits `onOpenChange(true)` while overlays remain closed until controlled `isOpen` updates (both picker variants).
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 97 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - expanded controlled keyboard-open request coverage to include `Alt+ArrowUp` behavior in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 99 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added disabled/read-only regression coverage ensuring click/keyboard open attempts do not emit `onOpenChange` requests.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - added dialog-wrapper regression coverage asserting popover dialog containers render with `dialogProps` ids in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - tightened dialog-wrapper accessibility coverage to assert `aria-labelledby` linkage on both picker dialog containers.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 109 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` parity update:
  - expanded SSR coverage for `CalendarDateTime` + minute-granularity rendering paths in both picker variants.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.ssr.test.ts`
...
- Validation: `npm test -- packages/@vue-spectrum/dialog/test` passed (5 files, 29 tests).
- Additional `@vue-spectrum/dialog` parity update:
  - expanded trigger/container type matrix coverage for `fullscreenTakeover` nested dialog sizing behavior.
    - `packages/@vue-spectrum/dialog/test/DialogTrigger.test.ts`
    - `packages/@vue-spectrum/dialog/test/DialogContainer.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/dialog/test` passed (5 files, 29 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` dynamic nested-data parity update:
  - added migrated coverage for dynamic `items` trees with multi-level expansion, validating visible row count and nested hierarchy attributes.
  - added dynamic-item DOM-prop forwarding coverage to verify custom row data attributes from item objects reach rendered tree rows.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 11 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` selection-semantics parity update:
  - added migrated coverage to verify expandable rows expose selection semantics (`aria-selected`) when tree selection mode is enabled.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` escape-key behavior parity update:
  - added `escapeKeyBehavior` prop support and forwarded it through `useTree` so selection clearing can be disabled.
  - added migrated regression coverage to verify `Escape` does not clear multi-selection when `escapeKeyBehavior="none"`.
  - `packages/@vue-spectrum/tree/src/TreeView.ts`
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` checkbox-aria parity update:
  - added migrated checkbox accessibility coverage for single-selection mode (`aria-label`, `aria-labelledby` linkage per row id) and multiple-selection tree `aria-multiselectable` semantics.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` empty-tabstop parity update:
  - added migrated coverage to verify empty trees remain tabbable (`tabindex="0"`) and render custom empty-state output.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` autofocus parity update:
  - added migrated coverage to verify `autoFocus="first"` moves focus to the first visible tree row.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 24 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` highlight-modifier parity update:
  - added migrated coverage for highlight-style multiple selection modifier behavior, verifying modifier-assisted row presses toggle additive selection state.
  - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 25 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` data-attribute passthrough parity update:
  ...
  - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 129 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` imperative-handle parity update:
  - exposed imperative component handles (`focus`, `blur`, `UNSAFE_getDOMNode`) on both `DatePicker` and `DateRangePicker`.
  - added regression coverage to verify imperative focus/blur behavior and DOM-node handle wiring for both picker variants.
  - `packages/@vue-spectrum/datepicker/src/DatePicker.ts`
  - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm run check -- --pretty false` passed.
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 131 tests).
- Additional `@vue-spectrum/datepicker` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` component-ref focus parity update:
  - added regression coverage to verify `DatePicker` and `DateRangePicker` imperative handles are callable through parent component refs for focus and DOM-node access.
  - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 135 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` MenuTrigger parity update:
  - expanded trigger behavior coverage for:
    - uncontrolled `defaultOpen` close semantics
    - `closeOnSelect={false}` selection flow preserving menu open-state and trigger expansion state
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` keyboard-navigation parity update:
  - expanded `Menu` interaction coverage for focus wrapping (`shouldFocusWrap`) across vertical arrow traversal.
  - stabilized menu test harness with `scrollIntoView` mocking for keyboard-driven focus movement branches.
  - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 22 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` selection-visibility parity update:
  - added `Menu` coverage asserting no selection checkmarks render when `selectionMode` is omitted, even with controlled `selectedKeys`.
  - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 23 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` docs parity update:
  - expanded menu docs with:
    - controlled/uncontrolled open-state patterns for `ActionMenu`
...
- Additional `@vue-spectrum/listbox` docs parity update:
  - expanded listbox docs with:
    - single-select default selection example
    - keyboard wrap guidance (`shouldFocusWrap`)
    - selection-indicator behavior note and key-shape compatibility note
  - `docs/packages/spectrum-listbox.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/listbox` selection-control parity update:
  - added single-selection coverage for:
    - uncontrolled `defaultSelectedKeys` transitions
    - controlled `selectedKeys` non-mutating selection behavior with `onSelectionChange`
  - `packages/@vue-spectrum/listbox/test/ListBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/listbox` disabled-navigation parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` falsy-key parity update:
  - added picker coverage for selecting falsy keys (`\"\"` and `0`) through hidden-select change flows.
  - fixed picker state value handling so empty-string keys are preserved when present in the collection instead of being treated as a cleared selection.
  - `packages/@vue-spectrum/picker/src/state.ts`
  - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 11 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` trigger-close parity update:
  - added picker coverage for closing the popup by pressing the trigger while open, including `onOpenChange(false)` and `aria-controls` teardown assertions.
  - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 12 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` docs parity update:
  - expanded picker docs with controlled/uncontrolled selection and open-state usage patterns.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` focus parity update:
  - added `autoFocus` coverage to verify trigger focus on mount.
  - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` form-attribute parity update:
  - added combobox coverage for `name` and `form` attribute propagation to the input element.
  - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 8 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` open-state parity update:
  - added controlled/uncontrolled open-state coverage for `isOpen` and `defaultOpen`, including close-attempt behavior via trigger interaction.
  - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 10 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` default-selection parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tabs` wrap-navigation parity update:
  - added horizontal ArrowLeft wrap coverage from first tab to last tab, including focus and selection assertions.
  - `packages/@vue-spectrum/tabs/test/Tabs.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tabs/test` passed (2 files, 9 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` sort-initialization parity update:
  - added `defaultSortDescriptor` coverage to verify initial sort direction state and row ordering.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 9 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` sort-callback parity update:
  - added repeated-header-press coverage to verify sort callback direction toggles from ascending to descending.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 12 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` multi-column sort-callback parity update:
  - added coverage to verify sort direction resets to ascending when switching from one sortable column to another.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 13 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` empty-state parity update:
  - added no-row coverage to verify `renderEmptyState` content renders in the table body empty cell.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 14 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` default-selection parity update:
  - added `defaultSelectedKeys` coverage to verify initial row `aria-selected` state in single-selection highlight mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 15 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-selection parity update:
  - added pointer-interaction coverage to verify rows listed in `disabledKeys` do not trigger `onSelectionChange`.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 16 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` unsafe-style parity update:
  - added `UNSAFE_style` passthrough coverage to verify inline style props merge onto the grid element.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 17 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled-sort parity update:
  - synchronized controlled `sortDescriptor` prop updates into internal sort state via reactive effect.
  - added regression coverage to verify parent prop updates retarget sorted column/order and rendered `aria-sort`.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled-selection interaction parity update:
  - added coverage to verify controlled row presses emit next selection payloads while rendered selection remains parent-driven until `selectedKeys` updates.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 22 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-keys update parity fix:
  - updated body row keying to remount rows when `disabledKeys` changes so row interactivity state refreshes with current disabled selection rules.
  - added regression coverage for dynamic `disabledKeys` switching.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 23 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
  - documented static-slot `Cell colSpan` behavior and ARIA colspan output.
  - `docs/packages/spectrum-table.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` data-driven colSpan parity update:
  - updated `items[].cells` normalization to preserve explicit `colSpan` cell grouping and column indexing in prop-driven tables.
  - added data-driven `colSpan` regression coverage for `columns` + `items` rendering.
  - `packages/@vue-spectrum/table/src/types.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 27 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled colSpan sort parity update:
  - added regression coverage for parent-driven `sortDescriptor` updates on data-driven rows with spanned cells.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 30 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` interactive colSpan sort parity update:
  - added regression coverage to verify header-press sort toggling (ascending/descending) on trailing columns in data-driven spanned rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 31 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` colSpan sort-callback parity update:
  - added regression coverage to verify `onSortChange` payloads remain correct for repeated interactive sorting on spanned trailing columns.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
  - documented static-slot row span-count validation requirements.
  - `docs/packages/spectrum-table.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
  ...
  - `docs/packages/spectrum-table.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` multiple-rowheader parity update:
  - added regression coverage to verify multiple `isRowHeader` columns render rowheader semantics with correct column indexing.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 35 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` checkbox-toggle parity update:
  - added regression coverage to verify repeated row presses in checkbox-style multi-selection toggle row selection on/off and emit the expected `onSelectionChange` key set.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 37 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` keyboard checkbox-selection parity update:
  - added regression coverage to verify checkbox-style multi-selection supports Space-key row selection and emits aggregated `onSelectionChange` key sets.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 38 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` keyboard-enter selection parity update:
  - added regression coverage to verify checkbox-style multi-selection supports Enter-key row selection and emits aggregated `onSelectionChange` key sets.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 39 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` single-checkbox selection parity update:
  - added regression coverage to verify `selectionMode: "single"` with `selectionStyle: "checkbox"` remains single-select when pressing multiple rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 40 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-checkbox selection parity update:
  - added regression coverage to verify checkbox-style multi-selection keeps disabled rows non-selectable while preserving selection callbacks for enabled rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 41 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` reactive disabled-checkbox parity update:
  - added regression coverage to verify checkbox-style multi-selection responds to runtime `disabledKeys` updates, including enabling/disabling rows across interactions.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 42 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` default checkbox-selection parity update:
  - added regression coverage to verify `defaultSelectedKeys` initializes checkbox-style multiple selection state for all configured rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled checkbox-selection parity update:
  - added regression coverage to verify controlled checkbox-style multiple selection reacts to runtime `selectedKeys` updates for add/remove transitions.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 44 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` checkbox select-all parity update:
  - added regression coverage to verify `Ctrl+A` selects all rows in checkbox-style multiple selection and emits a full selected-key set.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` escape no-op parity update:
  - added regression coverage to verify `Escape` does not emit `onSelectionChange` when checkbox-style multiple selection is already empty.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 48 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
  - documented disabled-row exclusion for checkbox-style `Ctrl+A` select-all callback payloads.
  - documented checkbox-style `Escape` clear/no-op behavior for non-empty vs empty selections.
  - `docs/packages/spectrum-table.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` single-mode select-all parity update:
  - added regression coverage to verify `Ctrl+A` is a no-op when `selectionMode` is `single` in checkbox-style selection.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 49 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` escape-key behavior parity update:
  - added `escapeKeyBehavior` prop support to `TableView` and forwarded it to underlying grid selection handling.
  - added regression coverage to verify `escapeKeyBehavior: "none"` preserves checkbox-style multiple selection on `Escape`.
  - documented `escapeKeyBehavior` in package docs.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` docs parity update:
  - clarified that both `disabledKeys` and item-level `isDisabled` rows are excluded from selection interactions and select-all key sets.
  - `docs/packages/spectrum-table.md`
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled select-all parity update:
  - added regression coverage to verify controlled checkbox-style select-all emits full key sets via `onSelectionChange` without mutating rendered controlled selection state.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 54 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled select-all opt-out parity update:
  - added regression coverage to verify controlled checkbox-style selection with `disallowSelectAll` suppresses `Ctrl+A` callbacks and preserves rendered controlled state.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 55 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled disallow-empty parity update:
  - added regression coverage to verify controlled checkbox-style selection with `disallowEmptySelection` suppresses deselection callbacks for the last selected row.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 56 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled escape-clear parity update:
  - added regression coverage to verify controlled checkbox-style `Escape` clearing emits an empty key set without mutating rendered controlled selection state.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 57 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled escape opt-out parity update:
  - added regression coverage to verify controlled checkbox-style selection with `escapeKeyBehavior: "none"` suppresses `Escape` callbacks and preserves rendered controlled state.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 58 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-behavior parity update:
  - added `disabledBehavior` prop support to `TableView` (`"selection"` | `"all"`), forwarding into table state.
  - added regression coverage to verify disabled rows remain actionable in `"selection"` mode and become fully non-actionable with `aria-disabled` in `"all"` mode.
  - documented `disabledBehavior` in package docs.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` duplicate-selection callback parity update:
  - added `allowDuplicateSelectionEvents` prop support to `TableView` and forwarded it to table-state selection handling.
  - added controlled-selection regression coverage for unchanged-selection callback suppression (default) and opt-in duplicate callbacks (`allowDuplicateSelectionEvents: true`).
  - documented `allowDuplicateSelectionEvents` in package docs.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 63 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` keyboard row-selection parity update:
  - added regression coverage for single-selection highlight mode on focused rows via `Space` and `Enter` key interactions.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 65 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled keyboard-selection parity update:
  - added regression coverage to verify disabled rows ignore `Space`/`Enter` keyboard selection attempts in single-selection highlight mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 66 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled checkbox keyboard-selection parity update:
  - added regression coverage to verify disabled rows ignore `Space`/`Enter` keyboard selection attempts in checkbox-style multiple mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 67 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` single-keyboard deselection parity update:
  - added regression coverage to verify `Space`/`Enter` clear the currently selected row in single highlight mode when empty selection is allowed.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 69 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` single-keyboard disallow-empty parity update:
  - added regression coverage to verify `disallowEmptySelection` preserves the selected row when pressing `Space`/`Enter` in single highlight mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 71 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` checkbox-keyboard disallow-empty parity update:
  - added regression coverage to verify `disallowEmptySelection` preserves the only selected row when pressing `Space`/`Enter` in checkbox-style multiple mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 72 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled checkbox unchanged-selection parity update:
  - added regression coverage to verify unchanged controlled checkbox selection attempts remain callback-suppressed when `disallowEmptySelection` prevents deselection.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 73 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-keys behavior parity update:
  - added regression coverage to verify `disabledBehavior` action gating semantics also apply when rows are disabled via `disabledKeys` (actionable in `"selection"`, non-actionable with `aria-disabled` in `"all"`).
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 75 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` checkbox press-up parity update:
  - added regression coverage to verify checkbox-style row selection is deferred until press-up/click when `shouldSelectOnPressUp` is true.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled single keyboard unchanged-selection parity update:
  - added regression coverage to verify unchanged controlled single-selection state remains callback-suppressed for `Space`/`Enter` keyboard attempts when `disallowEmptySelection` is true.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 78 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled checkbox keyboard unchanged-selection parity update:
  - added regression coverage to verify unchanged controlled checkbox selection remains callback-suppressed for `Space`/`Enter` keyboard attempts when `disallowEmptySelection` is true.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (2 files, 79 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled single keyboard duplicate-callback parity update:
  - added regression coverage to verify unchanged controlled single-selection keyboard attempts emit callbacks when `allowDuplicateSelectionEvents` is true.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` duplicate keyboard-enter parity update:
  - added regression coverage to verify unchanged controlled duplicate-selection callbacks also emit for `Enter` keyboard attempts in single highlight and checkbox selection modes when `allowDuplicateSelectionEvents` is true.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` visual-prop class parity update:
  - added `density` (`"compact" | "regular" | "spacious"`), `overflowMode` (`"truncate" | "wrap"`), and `isQuiet` props to `TableView`, wiring Spectrum root classes for default and custom visual variants.
  - added regression coverage for default classes (`spectrum-Table--regular`) and explicit quiet/compact/wrap class application.
  - documented visual props in package docs.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 89 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` column-alignment parity update:
  - added `columns[].align` support (`"start" | "center" | "end"`) through table column normalization and collection metadata.
  - applied alignment classes to both header and body cells.
  - added regression coverage for center/end alignment class output on headers and body cells.
  - documented column alignment support in package docs.
  - `packages/@vue-spectrum/table/src/types.ts`
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` static-slot column-metadata parity update:
  - added static-slot regression coverage to verify `align`, `hideHeader`, and `showDivider` metadata apply the same header/body classes as data-driven columns.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 92 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` row highlight-class parity update:
  - applied `spectrum-Table-row` class on body rows and wired `spectrum-Table-row--highlightSelection` to replace/highlight selection behavior.
  - added regression coverage for highlight mode (`selectionStyle="highlight"`) and checkbox mode (`selectionStyle="checkbox"`) row class behavior.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 94 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` spectrum cell-class parity update:
  - added `spectrum-Table-headCell` and `spectrum-Table-cell` classes on rendered header/body cells (including empty-state cells), alongside existing react-spectrum class hooks.
  - added regression coverage for baseline header/body spectrum cell classes.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 95 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` row position-class parity update:
  - added `spectrum-Table-row--firstRow` / `spectrum-Table-row--lastRow` class output for first/last body rows.
  - added regression coverage for first/last row class assignment.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 96 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` structural class parity update:
  - added Spectrum structural classes for table sections: `spectrum-Table-head`, `spectrum-Table-body`, and `spectrum-Table-headRow`.
  - aligned empty-state row class output to include `spectrum-Table-row`.
  - expanded regression coverage for header/body rowgroup classes, head-row class, and empty-row class behavior.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 96 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` hide-header body-cell class parity update:
  - extended `hideHeader` class behavior to body cells (`spectrum-Table-cell--hideHeader`) for both data-driven and static-slot column metadata.
  - expanded metadata class regressions to assert body-cell hide-header class output.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 96 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` static-column prop-normalization parity update:
  ...
  - `packages/@vue-spectrum/table/src/types.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 97 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` row state-class parity update:
  - added row state classes `is-selected` and `is-disabled` based on computed row selection and `aria-disabled` state.
  - expanded regression coverage to assert selected-row class toggling and disabled-row class output when `disabledBehavior` is `"all"`.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 97 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` root react-spectrum class parity update:
  - added `react-spectrum-Table` root class on `TableView` in addition to existing Spectrum and React-Spectrum table-view classes.
  - expanded visual-class regression coverage to assert the root `react-spectrum-Table` class.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 97 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` empty-row position-class parity update:
  - aligned empty-state fallback row classes to include both first/last row markers (`spectrum-Table-row--firstRow` and `spectrum-Table-row--lastRow`).
  - expanded empty-state regression coverage to assert empty-row first/last class output.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 97 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` sortable header state-class parity update:
  - added sortable/sorted header state classes (`is-sortable`, `is-sorted-asc`, `is-sorted-desc`) derived from column metadata and table sort state.
  - added regression coverage for sortable-class baseline and ascending/descending class transitions during sort toggles.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` static/data metadata normalization coverage update:
  - added static-slot regression coverage to verify kebab-case `Column` `allows-sorting` metadata enables sortable header behavior and sort ordering.
  - added data-driven regression coverage to verify string numeric `colSpan` values in row cell data normalize and render correct colspan/colindex output.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 102 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` static text-value sort-metadata coverage update:
  - added static-slot regression coverage to verify kebab-case `Cell` `text-value` metadata drives sort ordering independently of rendered text content.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 103 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` root visibility/disabled class coverage update:
  - added regression coverage for root class toggles driven by `isHidden` and `isDisabled` props.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-header keyboard parity update:
  - added regression coverage to verify disabled tables also suppress sortable-header keyboard sorting (`Enter`) callbacks and preserve unsorted header state.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 107 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` keyboard-navigation disablement parity update:
  - added `isKeyboardNavigationDisabled` support to `TableView`, wiring through table-state keyboard navigation disabling.
  - added regression coverage to verify Arrow/Home/End-style table key handling remains inert when keyboard navigation is disabled.
  - documented `isKeyboardNavigationDisabled` in package docs.
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
  - `docs/packages/spectrum-table.md`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled select-all no-op parity update:
  - added regression coverage to verify checkbox-style `Ctrl+A` is also a no-op when the table is globally disabled.
  - fixed translated `"all"` selection callback handling to suppress unchanged select-all callback emissions by default (while preserving opt-in duplicate-callback behavior via `allowDuplicateSelectionEvents`).
  - `packages/@vue-spectrum/table/src/TableView.ts`
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 112 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled select-all duplicate-callback parity update:
  - added regression coverage to verify disabled-table `Ctrl+A` still emits an empty-set `onSelectionChange` callback when `allowDuplicateSelectionEvents` is enabled.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 113 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled select-all unchanged-callback coverage update:
  - added controlled checkbox regression coverage to verify `Ctrl+A` does not emit `onSelectionChange` when all row keys are already selected.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 114 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-aria/grid` keyboard-disable branch coverage update:
  - added direct `useGrid` regression coverage to verify collection keyboard handlers are omitted (while focus/blur handlers remain) when `state.isKeyboardNavigationDisabled` is true.
  - `packages/@vue-aria/grid/test/useGrid.test.ts`
- Validation: `npm test -- packages/@vue-aria/grid/test packages/@vue-spectrum/table/test` passed (10 files, 142 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled keyboard-action parity update:
  - added regression coverage to verify disabled tables suppress row `onAction` keyboard activation (`Enter`) in addition to pointer activation.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 115 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` empty select-all duplicate-event guard coverage update:
  - added regression coverage to verify empty-table checkbox `Ctrl+A` remains a no-op even when `allowDuplicateSelectionEvents` is enabled.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (3 files, 116 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` action-expansion parity update:
  - added migrated coverage to verify expandable rows with `onAction` handlers fire actions on row press without toggling expansion.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` collapsed-link keyboard parity update:
  - added migrated keyboard-activation coverage to verify `Enter` on collapsed linked rows dispatches synthetic link clicks without expanding rows or emitting expansion callbacks.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` row-interactive arrow-key parity update:
  - added migrated keyboard coverage to verify `ArrowLeft`/`ArrowRight` traversal between row-interactive elements (row, checkbox, inline controls) while preserving row-focused collapse/expand semantics.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` empty-tree root-state parity update:
  - added root-level `data-empty` annotation for empty trees and expanded empty-tree interaction coverage to assert treegrid focusability and active-element behavior.
    - `packages/@vue-spectrum/tree/src/TreeView.ts`
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 52 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 52 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` base-attribute parity update:
  - added migrated coverage to verify baseline tree/root and row aria/data attributes in the default non-selection state, including absence of interaction-state data attributes before interaction.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` selectable-row action-metadata parity update:
  - expanded migrated selection-mode coverage to assert expandable rows omit `data-has-action` while remaining selectable (`aria-selected`) and expandable (`data-has-child-items`).
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` dynamic-row child-metadata parity update:
  - expanded migrated dynamic-tree coverage to assert `data-has-child-items` on expandable rows and absence on leaf rows across nested hierarchy levels.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` dynamic hierarchy-position parity update:
  - expanded migrated dynamic-tree coverage to assert nested `aria-posinset`/`aria-setsize` metadata on top-level and child rows alongside expansion-state checks.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` disabled-row metadata parity update:
  - expanded disabled-expansion coverage to assert `data-disabled` behavior on expandable rows (`disabledBehavior="all"` keeps metadata set, `disabledBehavior="selection"` keeps it unset while expansion remains enabled).
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` chevron disabled-attribute parity update:
  - expanded disabled-behavior chevron coverage to assert the native `disabled` attribute remains unset for `disabledBehavior="selection"` and is applied for `disabledBehavior="all"`.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 56 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 56 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` disabled-selection keyboard parity update:
  - added migrated coverage to verify `Enter` on disabled rows does not toggle expansion or emit expansion/selection callbacks when `selectionMode="multiple"` and `disabledBehavior="selection"`.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 57 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` selection-mode collapsed-state parity update:
  - expanded selection-mode expansion-suppression coverage to assert `data-expanded` stays unset when row/keyboard selection activation leaves expandable rows collapsed.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 57 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` disabled-all expanded keyboard parity update:
  - added migrated coverage to verify `Enter` on pre-expanded disabled rows does not collapse expansion, preserving `aria-expanded`/`data-expanded` and suppressing expansion callbacks under `disabledBehavior="all"`.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` empty-tree root-focus lifecycle parity update:
  - expanded empty-tree tabbability coverage to assert root focus-state annotations clear after blur (`data-focused`, `data-focus-visible`).
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 60 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 60 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` root focus-within parity update:
  - fixed root focus-state annotations to track focus within tree rows using focus-in/out lifecycle, not only direct root focus.
  - expanded populated-tree coverage to assert root `data-focused`/`data-focus-visible` behavior across keyboard and pointer modality transitions.
    - `packages/@vue-spectrum/tree/src/TreeView.ts`
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` highlight keyboard-replace parity update:
  - added keyboard activation coverage for `selectionMode="multiple"` + `selectionStyle="highlight"` to assert Enter-based replace selection semantics and no-op repeat activation behavior.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` single-highlight touch parity update:
  - added touch interaction coverage for single highlight mode to assert select/deselect behavior and callback payload transitions on repeated row activation.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 67 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` highlight keyboard-modifier toggle parity update:
  - added keyboard modifier (`Alt`/`Ctrl` + `Enter`) coverage in highlight multiple mode to assert additive toggle behavior and repeated-press deselection of the focused row.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 68 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` highlight touch data-attribute parity update:
  - expanded touch-toggle highlight coverage to assert `data-selected` row metadata tracks add/remove transitions alongside `aria-selected`.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 68 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 68 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` single-highlight keyboard-toggle parity update:
  - added keyboard activation coverage for single highlight mode to assert Enter-based select/deselect transitions and row-level `aria-selected`/`data-selected` updates.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` single-highlight pointer-replace parity update:
  - added single highlight pointer coverage to assert row-to-row replacement semantics and `aria-selected`/`data-selected` transitions under non-virtual mouse interaction.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 72 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` highlight keyboard DOM-metadata parity update:
  - expanded highlight keyboard replace coverage to assert `data-selected` transitions alongside `aria-selected` for both Enter activation and ArrowDown focus-move selection.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 72 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` collapsed-row Home/End parity update:
  - added keyboard navigation coverage to assert Home/End target first/last visible rows before and after collapsing a parent row.
    - `packages/@vue-spectrum/tree/test/TreeView.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` empty-state SSR parity update:
  - added SSR coverage to assert empty tree rendering preserves `data-empty` root metadata and semantic empty-state row/cell roles in server output.
    - `packages/@vue-spectrum/tree/test/TreeView.ssr.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/tree/test` passed (2 files, 79 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/tree` docs parity update:
  - expanded package docs with explicit empty-state and highlight-selection examples.
  - documented `renderEmptyState` in the key props matrix.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` submenu-selectedKeys parity update:
  - added migrated `SubmenuTrigger` coverage to assert submenu trigger rows are never selectable, even when their key is present in controlled `selectedKeys`.
  - asserted only selectable root items render `menuitemcheckbox` roles and checked state in `selectionMode="multiple"`.
    - `packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (7 files, 36 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` submenu trigger-neighbor hover-close parity update:
  - added migrated `SubmenuTrigger` coverage asserting pointer-hovering from the submenu trigger row onto a sibling root menu item closes the open submenu.
    - `packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts` passed (1 file, 14 tests).
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (7 files, 37 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` nested ArrowLeft close parity update:
  - added migrated `SubmenuTrigger` coverage asserting nested submenu triggers open to a third menu level and `ArrowLeft` on the focused nested trigger closes only the deepest submenu layer.
    - `packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts` passed (1 file, 20 tests).
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (7 files, 37 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` nested RTL ArrowRight close parity update:
  - added migrated `SubmenuTrigger` coverage asserting nested submenu triggers open via `ArrowLeft` in RTL and `ArrowRight` on the focused nested trigger closes only the deepest submenu layer.
    - `packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts` passed (1 file, 23 tests).
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 61 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (7 files, 37 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` deep submenu hover-close parity update:
  - added migrated `SubmenuTrigger` coverage asserting hovering from deepest submenu content to a root sibling item collapses nested submenu stacks back to the root menu.
  - stabilized nested RTL deep-close setup by opening the second-level submenu trigger via click before asserting `ArrowRight` closes only the deepest submenu layer.
    - `packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/SubmenuTrigger.test.ts` passed (1 file, 24 tests).
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (7 files, 37 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-aria/menu` submenu RTL locale-provider parity update:
  - added locale-integration `useSubmenuTrigger` coverage asserting RTL arrow-key semantics under `I18nProvider` (`ar-AE`):
    - `ArrowLeft` opens submenu triggers from the parent menu
    - `ArrowRight` closes open submenus and restores focus to the owning trigger
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` long-press selected-autofocus parity update:
  - expanded migrated `MenuTrigger` long-press coverage to assert opening via real long-press interaction autofocuses the selected menu item.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` small-screen portal-container parity update:
  - expanded migrated `MenuTrigger` portal coverage to assert overlays render in the provided portal container when `window.screen.width <= 700` (small-screen tray mode branch).
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` keyboard-tab retention parity update:
  - added migrated `MenuTrigger` coverage asserting `Tab` key navigation from a focused menu item does not dismiss the open root menu.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 77 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` typeahead focus parity update:
  - added migrated `Menu` coverage asserting rapid letter typing focuses matching items in sequence (`B` -> `L` -> `E`).
    - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 78 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` typeahead timeout/wrap parity update:
  - added migrated `Menu` coverage for typeahead buffer timeout reset and wrap-around focus behavior after timeout.
    - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 80 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` no-selection interaction parity update:
  - added migrated `Menu` coverage asserting `selectionMode="none"` ignores click/Enter/Space selection attempts and never emits `onSelectionChange`.
    - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 81 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` root aria-labelledby parity update:
  - added migrated `Menu` coverage asserting root-menu `aria-labelledby` wiring via the `ariaLabelledby` prop.
  - stabilized the test by removing brittle console-warning assertions and cleaning up mounted DOM labels.
    - `packages/@vue-spectrum/menu/test/Menu.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 82 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` root aria-label parity update:
  - added migrated `Menu` coverage asserting root-menu `aria-label` passthrough and absence of synthesized `aria-labelledby` when explicit labeling is provided.
    - `packages/@vue-spectrum/menu/test/Menu.test.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help hover parity update:
  - expanded migrated `MenuTrigger` coverage for contextual-help hover behavior:
    - unavailable contextual-help items open dialog content on delayed mouse hover
    - available contextual-help items (`isUnavailable: false`) do not open dialog content from hover
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 87 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help keyboard parity update:
  - added migrated `MenuTrigger` coverage asserting unavailable contextual-help items open dialog content via keyboard activation (`Enter`).
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 88 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help sibling-hover close parity update:
  - added migrated `MenuTrigger` coverage asserting contextual-help dialog overlays close when sibling root-menu items are hovered after the dialog opens.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 89 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help ArrowRight parity update:
  - added migrated `MenuTrigger` coverage asserting unavailable contextual-help items open dialog content when navigated with `ArrowRight`.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help rtl-arrow parity update:
  - added migrated `MenuTrigger` coverage asserting unavailable contextual-help items open dialog content via `ArrowLeft` navigation in RTL locale (`ar-AE`).
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 92 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help shift-tab containment parity update:
  - added migrated `MenuTrigger` coverage asserting contextual-help dialog focus remains contained when `Shift+Tab` is pressed from the focused dialog container, moving focus to the dialog link.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
  - updated dialog-mode submenu overlays to wrap content in `FocusScope` containment and added a targeted dialog-container `Shift+Tab` guard to focus the first tabbable child.
    - `packages/@vue-spectrum/menu/src/SubmenuTrigger.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 93 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help sibling-hover focus parity update:
  - added migrated `MenuTrigger` coverage asserting that when contextual-help closes from sibling-item hover, focus moves to the hovered sibling item.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
  - updated `useSubmenuTrigger` parent-menu hover-close path to focus the hovered parent menu item after closing an open submenu/dialog.
    - `packages/@vue-aria/menu/src/useSubmenuTrigger.ts`
...
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help reopen containment parity update:
  - added migrated `MenuTrigger` coverage asserting contextual-help focus containment still works after opening via click, closing on sibling hover, and reopening via hover.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/MenuTrigger.test.ts` passed (1 file, 33 tests).
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 97 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` trigger component-ref parity update:
  - added migrated `MenuTrigger` coverage asserting refs on both Spectrum `Button` and `MenuTrigger` components resolve to the same trigger DOM node.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/MenuTrigger.test.ts` passed (1 file, 34 tests).
- Validation: `npm test -- packages/@vue-spectrum/menu/test` passed (6 files, 98 tests).
- Validation: `npm test -- packages/@vue-aria/menu/test` passed (8 files, 43 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/menu` contextual-help rtl traversal parity update:
  - added migrated `MenuTrigger` coverage asserting unavailable contextual-help dialogs open in RTL after keyboard traversal (`ArrowDown`/`ArrowUp`) followed by `ArrowLeft` activation.
    - `packages/@vue-spectrum/menu/test/MenuTrigger.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/menu/test/MenuTrigger.test.ts` passed (1 file, 35 tests).
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/numberfield` textfield data-attribute parity update:
  - added migrated coverage asserting `data-*` attributes passed to `NumberField` are forwarded onto the text input element.
    - `packages/@vue-spectrum/numberfield/test/NumberField.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/numberfield` latin-numeral locale parity update:
  - added adapted coverage asserting latin numeral entry remains supported under arab locale (`ar-AE`) and commits with locale-aware latn formatting (`ar-AE-u-nu-latn`).
    - `packages/@vue-spectrum/numberfield/test/NumberField.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/numberfield/test` passed (2 files, 52 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/numberfield` arabic-numeral locale parity update:
  - added adapted coverage asserting arabic-indic numeral entry (`٢١`) commits correctly under arab locale (`ar-AE`) with arab numbering-system formatting (`ar-AE-u-nu-arab`).
    - `packages/@vue-spectrum/numberfield/test/NumberField.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/numberfield/test` passed (2 files, 53 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/numberfield` hanidec numeral parity update:
  - added adapted coverage asserting hanidec numeral entry (`二一`) parses and commits in default locale with `en-US-u-nu-hanidec` formatted output.
    - `packages/@vue-spectrum/numberfield/test/NumberField.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/numberfield` bengali numeral parity update:
  - added adapted coverage asserting bengali numeral entry (`২১`) parses and commits in default locale with `en-US-u-nu-beng` formatted output.
    - `packages/@vue-spectrum/numberfield/test/NumberField.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` disabled-mousedown parity update:
  - added migrated coverage ensuring disabled pickers do not open their menus on trigger `mousedown`.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` router external-link parity update:
  - extended RouterProvider link-item coverage to assert external links do not dispatch router `navigate` calls.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 36 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` placeholder warning parity update:
  - added migrated coverage asserting placeholder passthrough and placeholder deprecation warning logging.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` empty-loading parity update:
  - kept combobox popovers open for async loading states with empty collections by enabling `allowsEmptyCollection` whenever `loadingState` is provided.
    - `packages/@vue-spectrum/combobox/src/ComboBox.ts`
  - rendered loading placeholder rows for open comboboxes with no items in `loadingState="loading"` mode.
    - `packages/@vue-spectrum/combobox/src/ComboBox.ts`
  - added migrated coverage for loading placeholder rendering in empty async combobox menus.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` initial-load-more parity update:
  - added migrated coverage asserting `onLoadMore` is invoked when opening an underfilled combobox listbox viewport.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` docs form-value parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` input-attribute parity update:
  - expanded baseline render coverage to assert combobox input autocorrect/spellcheck/autocomplete semantics.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 28 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` async docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` form-value custom-input parity update:
  - added migrated coverage asserting `allowsCustomValue` forces text-form submission semantics even when `formValue="key"` is specified.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 31 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` aria-label parity update:
  - added migrated accessibility coverage for unlabeled combobox usage with `ariaLabel`.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` disabled-keys docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` manual-trigger open parity update:
  - added migrated coverage asserting `menuTrigger="manual"` opens the full option list when activated from the trigger button.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 42 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` enter-commit parity update:
  - added migrated keyboard coverage asserting Enter commits the currently focused combobox option and closes the menu.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` selected-item open-change parity update:
  - added migrated coverage asserting selectedKey-controlled comboboxes emit close callbacks when users select the currently selected option.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` focus-trigger clear-input parity update:
  - added migrated coverage asserting clearing typed input with `menuTrigger=\"focus\"` keeps the menu open and clears active descendant focus.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` default initialization parity update:
  - added migrated coverage asserting:
    - `defaultInputValue` alone does not imply a selected key
    - `defaultSelectedKey` initializes the input value text
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 63 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` reselect-close parity update:
  - added migrated coverage asserting uncontrolled combobox menus close when the user clicks an already-selected option.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 64 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` escape-revert parity update:
  - added migrated coverage asserting Escape resets typed input to the selected item text and closes the menu.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 65 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` selected-option reselect parity update:
  - added migrated coverage asserting reselecting an already selected option resets input text for both pointer and Enter-key commit paths.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 67 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` blur-retain parity update:
  - added migrated coverage asserting selected-key form value is retained after blur when typed input is restored to the selected item text.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` shift-tab commit parity update:
  - added migrated keyboard coverage asserting Shift+Tab commits the focused option and closes the menu.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` default-input arrow-open parity update:
  - added migrated coverage asserting ArrowDown-open shows all options even when `defaultInputValue` is preset.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` default-input re-filter parity update:
  - added migrated coverage asserting the open menu re-filters options when input text changes after opening.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 83 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` controlled-items trigger-open parity update:
  - added migrated coverage asserting trigger-open preserves an externally controlled filtered `items` list rather than expanding to all source items.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` manual-trigger blur-clear parity update:
  - added migrated coverage asserting unmatched input clears on blur when `menuTrigger="manual"` and the menu remains closed.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 86 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` disabled-match blur-clear parity update:
  - added migrated coverage asserting blur clears typed text when it matches a disabled option.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 87 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` option-mousedown focus parity update:
  - added migrated coverage asserting combobox input focus is retained when mousing down on an open menu option.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` Tab focus-transition parity update:
  - added migrated coverage asserting Tab commits the focused option while allowing focus to continue to the next control.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 90 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` Shift+Tab focus-transition parity update:
  - added migrated coverage asserting Shift+Tab commits the focused option while allowing focus to continue to the previous control.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` controlled-selectedKey clear-input parity update:
  - added migrated coverage asserting clearing text emits `onSelectionChange(null)` when `selectedKey` is controlled while retaining controlled selected option semantics.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 94 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` blur-propagation parity update:
  - added migrated coverage asserting outside focus transitions trigger combobox blur callbacks while still propagating parent focus-out handlers.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 95 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` matching-defaults parity update:
  - added migrated coverage asserting matching `defaultSelectedKey` and `defaultInputValue` initialize both the visible input value and hidden key-form value.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` uncontrolled input/selection flow parity update:
  - added migrated coverage asserting uncontrolled input typing, clearing, and option selection update callbacks/state in upstream order.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` form-reset parity update:
  - added migrated coverage asserting selected combobox text resets when the parent form reset control is activated.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` aria-validation parity update:
  - added migrated coverage for `validate` callback and server-provided form validation errors in `validationBehavior="aria"` flows.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
  - wired combobox `validate` and `name` props into combobox state validation so client/server validation paths are active.
    - `packages/@vue-spectrum/combobox/src/types.ts`
    - `packages/@vue-spectrum/combobox/src/ComboBox.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` aria server-validation clear parity update:
  - added migrated coverage asserting aria-mode server validation clears after selecting a valid option and blurring.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 111 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` native server-validation clear parity update:
  - added migrated coverage asserting native-mode server validation clears after selecting a valid option and blurring.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 112 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` native blur-commit parity update:
  - added migrated coverage asserting native validation is not committed on blur when the value is unchanged, and is committed on explicit form validation.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 113 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` native required-clear parity update:
  - added migrated coverage asserting native required validation clears after selecting a valid option and blurring.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
  - fixed native validation refresh wiring so programmatic value updates (e.g. combobox option selection) recompute and commit current native validity.
    - `packages/@vue-aria/form/src/useFormValidation.ts`
    - `packages/@vue-aria/textfield/src/useTextField.ts`
    - `packages/@vue-aria/combobox/src/useComboBox.ts`
- Validation: `npm test -- packages/@vue-spectrum/combobox/test` passed (2 files, 114 tests).
- Validation: `npm test -- packages/@vue-spectrum/numberfield/test` passed (2 files, 87 tests).
- Validation: `npm test -- packages/@vue-spectrum/textfield/test` passed (4 files, 27 tests).
- Validation: `npm test -- packages/@vue-aria/form/test` passed (2 files, 18 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` validation docs parity update:
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` native invalid-focus parity update:
  - added migrated coverage asserting failed native form validation focuses the combobox input as the first invalid control.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` blur-callback parity update:
  - added migrated coverage asserting picker selection does not spuriously call external `onBlur` callbacks.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` native reset-validation parity update:
  - added migrated coverage asserting native picker validation UI clears after form reset.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` aria-label parity refinement:
  - added migrated coverage asserting trigger labeling via `aria-label`.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 60 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` visible-label parity refinement:
  - added migrated coverage asserting trigger/listbox labeling semantics when a visible label is provided.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 61 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` hover-focus parity refinement:
  - added migrated coverage asserting open picker options receive focus on hover when pointer modality is active.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 62 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` disabled-option navigation parity refinement:
  - added migrated coverage asserting open keyboard navigation skips disabled options.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/picker/test` passed (2 files, 63 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/picker` open typeahead parity refinement:
  - added migrated coverage asserting open-listbox typeahead moves focus across matching options.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Additional `@vue-spectrum/picker` keyboard-selection parity refinement:
  - added migrated coverage asserting:
    - open picker options can be selected with both `Space` and `Enter` keyboard activation.
    - open-list keyboard focus traversal works with options keyed by empty-string values.
    - `packages/@vue-spectrum/picker/test/Picker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` form-reset parity refinement:
  - added migrated coverage asserting date picker form reset restores hidden-input serialization back to the configured default date after calendar selection changes.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
  - wired hidden-input `useFormReset` handling in both date picker and range picker spectrum wrappers so native form reset events restore stately values from `defaultValue`.
    - `packages/@vue-spectrum/datepicker/src/DatePicker.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` form-action default-value parity refinement:
  - added migrated coverage asserting uncontrolled date picker `defaultValue` updates are reflected after form submit action flows.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
  - synchronized uncontrolled date picker/range picker state when `defaultValue` changes by watching default props in stately state hooks and applying updates only when not controlled.
    - `packages/@vue-aria/datepicker-state/src/useDatePickerState.ts`
    - `packages/@vue-aria/datepicker-state/src/useDateRangePickerState.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 138 tests).
- Validation: `npm test -- packages/@vue-aria/datepicker-state/test packages/@vue-spectrum/datepicker/test` passed (6 files, 152 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` range form-action default-value parity refinement:
  - added migrated coverage asserting uncontrolled range picker `defaultValue` updates are reflected after form submit action flows.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 139 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` controlled form-reset parity refinement:
  - added migrated coverage asserting controlled date picker values reset to their initial form baseline after native form reset.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 140 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/datepicker` controlled range form-reset parity refinement:
  - added migrated coverage asserting controlled range picker values reset to their initial form baseline after native form reset.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-aria/datepicker-state` controlled-mode defaultValue guard coverage:
  - added regression tests asserting `defaultValue` prop updates do not override controlled `value` state for date picker and range picker hooks.
    - `packages/@vue-aria/datepicker-state/test/useDatePickerState.test.ts`
- Validation: `npm test -- packages/@vue-aria/datepicker-state/test packages/@vue-spectrum/datepicker/test` passed (6 files, 159 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/combobox` loading-state timer parity coverage:
  - expanded migrated loading-state tests to cover additional upstream timer and visibility semantics:
    - filtering spinner hides when the menu closes.
    - delayed spinner timer is canceled when loading finishes before 500ms.
    - delayed spinner timer is not reset when `loadingState` transitions from `loading` to `filtering`.
    - filtering spinner behavior coexists with invalid-state rendering without introducing listbox loading spinners for filtering mode.
    - `packages/@vue-spectrum/combobox/test/ComboBox.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-aria/combobox-state` default-key sync regression coverage:
  - added state-hook regression tests asserting:
    - uncontrolled combobox state syncs to reactive `defaultSelectedKey` prop changes after mount.
    - controlled `selectedKey` mode ignores `defaultSelectedKey` updates (no uncontrolled override).
...
- Additional `@vue-spectrum/datepicker` native `aria-describedby` parity refinement:
  - added fallback generated error IDs for date picker and range picker error-message nodes and wired dynamic group-level `aria-describedby` merging so native validation errors are programmatically associated with group semantics.
    - `packages/@vue-spectrum/datepicker/src/DatePicker.ts`
  - expanded migrated native-validation coverage to assert:
    - custom native error messages are discoverable through group `aria-describedby` references for date and range pickers.
    - native required error descriptions are cleared from group `aria-describedby` after calendar selection resolves validity.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
  - stabilized native invalid-commit tests by awaiting initial validation effect ticks before firing invalid events, matching Vue lifecycle timing for hidden native-validation inputs.
    - `packages/@vue-spectrum/datepicker/test/DatePicker.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/datepicker/test` passed (2 files, 153 tests).
- Validation: `npm test -- packages/@vue-aria/datepicker-state/test packages/@vue-spectrum/datepicker/test` passed (6 files, 171 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` treegrid hierarchy parity coverage expansion:
  - added migrated coverage for parent-gated expansion semantics:
    - child expanded keys alone do not materialize nested descendants unless ancestor keys are also expanded.
  - added migrated coverage for multi-row-header expander placement:
    - expander renders on the first row-header cell when multiple row headers are present.
  - `packages/@vue-spectrum/table/test/TableNestedRows.test.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 126 tests).
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` default-width template metadata coverage:
  - added kebab-case static-slot coverage for `default-width` metadata to ensure Vue template parity for default sizing behavior.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` controlled-resize callback parity coverage:
  - added integration coverage asserting controlled `width` columns remain visually stable during keyboard resize attempts until parent props change.
  - asserted resize callbacks still emit updated width maps during the controlled flow.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` touch-resize interaction parity coverage:
  - added integration coverage for touch-pointer drag resize flow (`pointerType: "touch"`), including width updates and resize callback assertions.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` resizable-column update/reset parity coverage:
  - added integration coverage asserting uncontrolled resizable width state resets correctly when column sets change (remove/add cycle), matching expected default fractional distributions after each update.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 178 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 234 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` wrap-overflow controlled-selection regression coverage:
  - added integration coverage ensuring controlled selection-mode transitions remain stable and non-throwing when `overflowMode="wrap"` is enabled.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` keyboard range-selection coverage expansion:
  - added regression coverage for modifier-extended range selection via `Ctrl+Shift+Home` in checkbox-style multiple-selection tables.
  - verifies full-range accumulation from a lower focused row back to the first row using keyboard-only interaction.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` pointer range-selection coverage expansion:
  - added reverse-direction checkbox `Shift+click` range-selection coverage (selecting from a lower row back to an earlier row).
  - verifies contiguous multi-row accumulation is symmetric regardless of pointer range direction.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` select-all guard coverage expansion:
  - added larger-collection (`3` rows) regression coverage ensuring `disallowSelectAll` continues to block `Ctrl+A` select-all keyboard interaction.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 189 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 245 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` disabled-selection breadth coverage expansion:
  - added larger-collection (`3` rows) regression coverage ensuring `Ctrl+A` select-all excludes disabled keys while still selecting enabled rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 190 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 246 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` item-disabled select-all breadth expansion:
  - added larger-collection (`3` rows) regression coverage ensuring `Ctrl+A` select-all excludes item-level disabled rows (`isDisabled`) while selecting enabled rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 191 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 247 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` single-selection select-all guard breadth:
  - added larger-collection (`3` rows) regression coverage ensuring checkbox single-selection mode continues to ignore `Ctrl+A` select-all requests.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 192 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 248 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` escape-clear breadth coverage:
  - added larger-collection (`3` rows) regression coverage ensuring checkbox multi-selection clear-on-Escape behavior resets all selected rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 193 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 249 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` escape-none breadth coverage:
  - added larger-collection (`3` rows) regression coverage ensuring `escapeKeyBehavior="none"` preserves checkbox multi-selection state and suppresses escape callbacks.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 194 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 250 tests).
- Validation: `npm run check -- --pretty false` passed.
...
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 241 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` range/select-all guard coverage expansion:
  - added regression coverage ensuring shift-click pointer range selection excludes disabled rows inside the selected span.
  - added regression coverage ensuring deselecting a row after full select-all updates selection to the remaining rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` select-all dynamic-item parity coverage:
  - added regression coverage ensuring newly added rows are automatically selected when select-all is active.
  - added regression coverage ensuring manually selecting all existing rows does not auto-select newly added rows.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` select-all header mode guard coverage:
  - added regression coverage ensuring select-all checkbox is disabled in checkbox single-selection mode.
  - added regression coverage ensuring select-all checkbox is not rendered when selection mode is `none`.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` row-link keyboard activation coverage:
  - added Provider router integration regression coverage ensuring Enter-key activation of linked rows triggers router `navigate` in `selectionMode="none"`.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` treegrid pointer-range selection parity expansion:
  - added migrated nested-row pointer range-selection coverage for:
    - top-level to descendant Shift+click ranges.
    - descendant to top-level Shift+click ranges.
    - disabled-row exclusion during pointer Shift+click range extension.
  - `packages/@vue-spectrum/table/test/TableNestedRows.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` treegrid checkbox-keyboard parity expansion:
  - added migrated nested-row coverage for checkbox-focused keyboard selection semantics:
    - Enter on a focused row checkbox selects the corresponding nested row.
    - Space on a focused row checkbox selects the corresponding nested row.
  - `packages/@vue-spectrum/table/test/TableNestedRows.test.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` drag-label i18n parity expansion:
  - added regression coverage ensuring drag-handle `aria-label` text localizes via `I18nProvider` locale overrides.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 317 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` non-treegrid empty/loading colspan drag parity expansion:
  - added regression coverage ensuring empty-state and loading spinner rows account for both drag and selection synthetic columns (`aria-colspan`/`aria-colcount` = `5`) in standard grid mode.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
- Validation: `npm test -- packages/@vue-spectrum/table/test` passed (4 files, 263 tests).
- Validation: `npm test -- packages/@vue-aria/table-state/test packages/@vue-aria/table/test packages/@vue-spectrum/table/test` passed (18 files, 319 tests).
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` non-treegrid loadingMore colspan drag parity expansion:
  - added regression coverage ensuring `loadingMore` bottom spinner rows also account for drag + selection synthetic columns (`aria-colspan`/`aria-colcount` = `5`).
  - `packages/@vue-spectrum/table/test/TableTests.ts`
...
- Validation: `npm run check -- --pretty false` passed.
- Additional `@vue-spectrum/table` slot load-more drag parity expansion:
  - added regression coverage ensuring `TableBody` slot `onLoadMore` callbacks still trigger when drag + selection synthetic columns are active.
  - `packages/@vue-spectrum/table/test/TableTests.ts`
